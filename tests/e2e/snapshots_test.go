...
func CreateVirtualDiskSnapshot(vdName, volumeSnapshotClassName string, requiredConsistency bool, labels map[string]string) error {
	GinkgoHelper()
	vdSnapshotName := fmt.Sprintf("%s-%s", vdName, volumeSnapshotClassName)
	...
	filePath := fmt.Sprintf("%s/vd-snapshots/%s-%s.yaml", conf.TestData.Snapshots, vdName, volumeSnapshotClassName)
...
func createImmediateStorageClass(provisioner string, labels map[string]string) (string, error) {
	GinkgoHelper()
	filePath := fmt.Sprintf("%s/immediate-storage-class.yaml", conf.TestData.Snapshots)
	switch provisioner {
	case "replicated.csi.storage.deckhouse.io":
		replicatedStorageClassName := fmt.Sprintf("%s-linstor-immediate", namePrefix)
		replicatedStoragePoolName, err := GetCompletedReplicatedStoragePool()
		if err != nil {
			return "", err
		}
...
// Get first replicated storage pool with phase `Completed`
func GetCompletedReplicatedStoragePool() (string, error) {
	GinkgoHelper()
	...
	for _, rsp := range rspObjects.Items {
		if rsp.Status.Phase == PhaseCompleted {
			return rsp.Name, nil
		}
	}
	return "", fmt.Errorf("cannot get completed replicated storage pool")
}
...
func CheckFilesystemReadyStatus(vmName string, status v1.ConditionStatus) (string, error) {
	GinkgoHelper()
	...
	for _, condition := range vmObj.Status.Conditions {
		if condition.Type == "FilesystemReady" {
			if condition.Status != status {
				return condition.Reason, fmt.Errorf("`FilesystemReady` status of %q is not %q: %s", vmName, status, condition.Reason)
			} else {
...
var _ = Describe("VMs and VDs snapshots", ginkgoutil.CommonE2ETestDecorators(), func() {
	var (
		immediateStorageClassName      string // require for unattached virtual disk snapshots
		defaultVolumeSnapshotClassName string
		testCaseLabel                  = map[string]string{"testcase": "snapshots"}
		attachedVirtualDiskLabel       = map[string]string{"attachedVirtualDisk": ""}
		// attachedVirtualMachineBlockDeviceAttachmentLabel = map[string]string{"attachedVirtualMachineBlockDeviceAttachment": ""}
		hasNoConsumerLabel = map[string]string{"hasNoConsumer": "snapshots"}
	)
	...
	Context("Environment preparing", func() {
		It("prepares `Immediate` storage class and virtual disk that use it", func() {
			...
			if *sc.VolumeBindingMode != storagev1.VolumeBindingImmediate {
				...
				vdWithoutConsumerFilePath := fmt.Sprintf("%s/vd/vd-alpine-http.yaml", conf.TestData.Snapshots)
	...
	Context("When virtualization resources are applied:", func() {
		It("result should be succeeded", func() {
			res := kubectl.Apply(kc.ApplyOptions{
				Filename:       []string{conf.TestData.Snapshots},
	...
	Context(fmt.Sprintf("When unattached VDs in phase %s:", PhaseReady), func() {
		It("creates VDs snapshots with `requiredConsistency`", func() {
			...
			for _, vdName := range vds {
				...
				err := CreateVirtualDiskSnapshot(vdName, volumeSnapshotClassName, true, hasNoConsumerLabel)
				Expect(err).NotTo(HaveOccurred(), "%s", err)
			}
		})
	...
	Context(fmt.Sprintf("When virtual machines in %s phase", PhaseRunning), func() {
		It("creates snapshots with `requiredConsistency` of attached VDs", func() {
			...
			for _, vm := range vmObjects.Items {
				Eventually(func() error {
					_, err := CheckFilesystemReadyStatus(vm.Name, v1.ConditionTrue)
					return err
				}).WithTimeout(
				...
				for _, blockDevice := range blockDevices {
					if blockDevice.Kind == virtv2.VirtualDiskKind {
						...
						err := CreateVirtualDiskSnapshot(blockDevice.Name, defaultVolumeSnapshotClassName, true, attachedVirtualDiskLabel)
						Expect(err).NotTo(HaveOccurred(), "%s", err)
						Eventually(func() error {
							reason, err := CheckFilesystemReadyStatus(vm.Name, v1.ConditionFalse)
							if reason != "Frozen" {
		...
		It("checks `FileSystemReady` status of VMs", func() {
			...
			for _, vm := range vmObjects.Items {
				Eventually(func() error {
					reason, err := CheckFilesystemReadyStatus(vm.Name, v1.ConditionTrue)
					if err != nil {
	...
	Context("When test is completed", func() {
		It("deletes test case resources", func() {
			DeleteTestCaseResources(ResourcesToDelete{
				KustomizationDir: conf.TestData.Snapshots,
