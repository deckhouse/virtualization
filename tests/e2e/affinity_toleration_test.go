...
var _ = Describe("VirtualMachineAffinityAndToleration", ginkgoutil.CommonE2ETestDecorators(), func() {
	...
	Context("When the virtualization resources are applied:", func() {
		...
		It("checks the resources phase", func() {
			By(fmt.Sprintf("`VirtualImages` should be in the %q phase", virtv2.ImageReady))
			...
			By(fmt.Sprintf("`VirtualMachineClasses` should be in %s phases", virtv2.ClassPhaseReady))
			...
			By(fmt.Sprintf("`VirtualDisks` should be in the %q phase", virtv2.DiskReady))
			...
			By("`VirtualMachines` agents should be ready")
