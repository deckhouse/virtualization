...
var _ = Describe("VirtualMachineLiveMigrationTCPSession", SIGMigration(), framework.CommonE2ETestDecorators(), func() {
	var (
		ctx    context.Context
...
func WaitForVirtualMachine(ctx context.Context, namespace, name string, h EventHandler[*v1alpha2.VirtualMachine]) *v1alpha2.VirtualMachine {
	GinkgoHelper()

	virtualMachine, err := WaitFor(ctx, framework.GetClients().VirtClient().VirtualMachines(namespace), h, metav1.ListOptions{
...
func WaitForVirtualDisk(ctx context.Context, namespace, name string, h EventHandler[*v1alpha2.VirtualDisk]) *v1alpha2.VirtualDisk {
	GinkgoHelper()

	virtualDisk, err := WaitFor(ctx, framework.GetClients().VirtClient().VirtualDisks(namespace), h, metav1.ListOptions{
		FieldSelector: fields.OneTermEqualSelector("metadata.name", name).String(),
	})
	Expect(err).NotTo(HaveOccurred())
