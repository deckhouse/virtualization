...
func NewClient(remoteStream v1alpha2.StreamInterface, usbRedirector *usbRedirector) *Client {
...
func (c *Client) Redirect(ctx context.Context, device string) error {
	...
	go func() {
		defer close(c.done)
		usbRedirectorCh <- c.usbRedirector.Redirect(ctx, device, address)
	}()
...
func newUsbRedirector(bin string, sudo bool) *usbRedirector {
	return &usbRedirector{
		bin:  bin,
		sudo: sudo,
	}
}

func (u *usbRedirector) Redirect(ctx context.Context, device, address string) error {
	var (
		command string
		args    []string
	)
