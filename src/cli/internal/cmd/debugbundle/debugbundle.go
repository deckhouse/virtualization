...
func NewCommand() *cobra.Command {
	bundle := &DebugBundle{}
	cmd := &cobra.Command{
		Use:     "debug-bundle (VirtualMachine)",
		Short:   "Create a debug bundle with VM configuration, events, and logs.",
		Example: usage(),
		Args:    templates.ExactArgs("debug-bundle", 1),
		RunE:    bundle.Run,
	}

	cmd.Flags().BoolVar(&bundle.saveLogs, "save-logs", false, "Save pod logs to output")
	cmd.Flags().BoolVar(&bundle.saveSystemLogs, "save-system-logs", false, "Save system component logs to output")
...
func usage() string {
	return `  # Create debug bundle for VirtualMachine 'myvm':
  {{ProgramName}} debug-bundle myvm
  {{ProgramName}} debug-bundle myvm.mynamespace
  {{ProgramName}} debug-bundle myvm -n mynamespace
  # Include pod logs:
  {{ProgramName}} debug-bundle --save-logs myvm
  # Include system component logs:
  {{ProgramName}} debug-bundle --save-system-logs myvm`
}
...
func (b *DebugBundle) collectResources(ctx context.Context, client kubeclient.Client, namespace, vmName string) error {
	...
	if b.saveSystemLogs {
		if err := b.collectSystemLogs(ctx, client); err != nil {
			return fmt.Errorf("failed to collect system logs: %w", err)
		}
	}
