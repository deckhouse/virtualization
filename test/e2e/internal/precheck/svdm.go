...
const (
	// svdModuleName is the name of the Storage Volume Data Manager module in Deckhouse.
	svdModuleName         = "storage-volume-data-manager"
	svdmModuleCheckEnvName = "SVDM_MODULE_PRECHECK"
)

// svdmPrecheck implements PrecheckWithEnv interface for SVDM module.
...
func (s *svdmPrecheck) EnvName() string {
	return svdmModuleCheckEnvName
}
...
func (s *svdmPrecheck) Run(ctx context.Context, f *framework.Framework) error {
	...
	err := f.GenericClient().Get(ctx, client.ObjectKey{Name: svdModuleName}, svdmModule)
	if err != nil {
