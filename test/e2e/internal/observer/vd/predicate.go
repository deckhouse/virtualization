...
// provisioning, VirtualDisk.Status.Progress stays at the same value for longer
// than threshold before advancing.
//
// The intent is to catch progress that "jumps" instead of streaming smoothly,
// for example a disk whose progress sits at 0% for a long time and then jumps
...
//
// Only the Provisioning phase is inspected: the Pending, WaitForFirstConsumer
// and WaitForUserUpload phases legitimately sit at 0% (the disk is waiting for
// a consumer or for the user upload to begin) and are skipped. Reaching 100% is
// treated like any other advance, so a long stall just before completion (e.g.
// 50% -> 100%) is also reported. Intended for use with [Observer.Always].
