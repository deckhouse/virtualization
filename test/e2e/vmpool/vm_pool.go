...
import (
	...
	vmpoolobs "github.com/deckhouse/virtualization/test/e2e/internal/observer/vmpool"
	"github.com/deckhouse/virtualization/test/e2e/internal/precheck"
)

var _ = Describe("VirtualMachinePool", Label(label.SIGCompute, precheck.NoPrecheck), func() {
	var (
	...
	BeforeEach(func() {
		...
		f = framework.NewFramework("vm-pool")
	...
	buildPool := func(replicas int32, policy v1alpha2.ScaleDownPolicy, reclaim v1alpha2.VirtualDiskReclaim) *v1alpha2.VirtualMachinePool {
		tmpl := object.NewMinimalVM("", f.Namespace().Name,
			// The pool boots the myos image, not the custom one NewMinimalVM is
			// sized for.
	...
	It("maintains the requested number of tiny replicas, each with its own root disk, and scales", func() {
		var poolObs vmpoolobs.Observer
		By("Creating a pool of 2 tiny VMs with a per-replica root disk from the myos image", func() {
			...
			// The pool uses generateName, so the observer can only start once the
			// name is assigned by the create call.
			poolObs = vmpoolobs.StartObserver(ctx, f, pool)
		})

		By("Waiting until both replicas are Running", func() {
			// status.readyReplicas is the pool's own count of Running members.
			err := poolObs.WaitFor(vmpoolobs.HaveReadyReplicas(2), framework.LongTimeout)
		...
		By("Waiting until all 3 replicas are Running", func() {
			err := poolObs.WaitFor(vmpoolobs.HaveReadyReplicas(3), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
		})
	})
	...
	It("removes addressed replicas via scaleDownWith, shrinks the pool, and does not replace them", func() {
		...
		By("Verifying the pool shrank to 1, spec.replicas was decremented, and no replacement appears", func() {
			err := poolObs.WaitFor(vmpoolobs.HaveReplicas(1), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
			Expect(f.GenericClient().Get(ctx, crclient.ObjectKeyFromObject(pool), pool)).To(Succeed())
			Expect(ptr.Deref(pool.Spec.Replicas, -1)).To(Equal(int32(1)))
			// Unlike a plain delete, scaleDownWith is not a lost replica: the removed
			// VM must not come back. The invariant is checked against every pool
			// status update from here until the end of the spec, so any replacement
			// the controller creates fails the spec via the observer's cleanup.
			poolObs.Never(vmpoolobs.HaveMoreReplicasThan(1))
			for _, m := range members() {
