...
var _ = Describe("VirtualMachineMigrationDedicatedNetwork", Label(label.SIGCompute, precheck.PrecheckSDN), func() {
	var (
	...
	It("routes live migration traffic over the configured SystemNetwork", func() {
		...
		By("Waiting for migration-iface annotations to populate on every node", func() {
			// EXCEPTION: no observer package exists for core Nodes, and this wait
			// spans the whole node list at once; converting would require a new
			// shared observer package — left as a polling wait deliberately.
			...
			}, framework.MiddleTimeout, eventually.WithPolling(2*time.Second))
		})

		By("Creating a VM", func() {
			// The root disk size is not tied to the image: the custom image
			// is ~35Mi and grows its filesystem to the disk on first boot.
			...
			vmObs = vmobs.StartObserver(ctx, f, vmObj)
			...
			Expect(err).NotTo(HaveOccurred())
			eventually.SSHReadyAsRoot(f, vmObj, framework.LongTimeout)
		})
		...
		By("Triggering live migration via VMOP", func() {
			...
			// The helper waits for the VMOP to complete with the migration
			// fast-fail watchers armed (failed/wedged migrations abort the wait
			// immediately) and already skips the known migration issues.
			util.UntilVMOPMigrationSucceeded(ctx, vmop, framework.LongTimeout)
		})
