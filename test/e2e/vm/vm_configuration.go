...
var _ = Describe("VirtualMachineConfiguration", Label(label.SIGCompute, precheck.NoPrecheck), func() {
	DescribeTable("the configuration should be applied", func(restartApprovalMode v1alpha2.RestartApprovalMode) {
		...
		// With Manual restart approval the controller only marks the VM as
		// awaiting a restart; the reboot must be requested explicitly.
		if t.VM.Spec.Disruptions.RestartApprovalMode == v1alpha2.Manual {
			err := vmObs.WaitFor(vmobs.BeAwaitingRestart(), 10*time.Second)
			Expect(err).NotTo(HaveOccurred())
			util.RebootVirtualMachineByVMOP(f, t.VM)
		}

		By("Waiting for VM to be rebooted")
		key := crclient.ObjectKeyFromObject(t.VM)
		...
		// After the reboot no restart may be pending anymore. The invariant is
		// enforced by the observer on every VM update from here until the end of
		// the spec, replacing the previous bounded Consistently window.
		_, exists := conditions.GetCondition(vmcondition.TypeAwaitingRestartToApplyConfiguration, t.VM.Status.Conditions)
		Expect(exists).To(BeFalse())
		Expect(t.VM.Status.RestartAwaitingChanges).To(BeNil())
		vmObs.Never(vmobs.BeAwaitingRestart())
	},
...
func (t *configurationTest) GenerateResources(restartApprovalMode v1alpha2.RestartApprovalMode) {
	t.VDRoot = object.NewVDFromCVI("vd-root", t.Framework.Namespace().Name, object.PrecreatedCVICustomBIOS, vdbuilder.WithSize(ptr.To(resource.MustParse(vdCustomImageSize))))
