...
var _ = Describe("VirtualMachineAffinityAndToleration", Label(label.SIGCompute, precheck.PrecheckAffinityToleration), func() {
	var (
		...
		vmAObs vmobs.Observer
		vmCObs vmobs.Observer
	)

	BeforeEach(func() {
		f = framework.NewFramework("vm-affinity-and-toleration")
	...
	It("checks placement via status.nodeName and migrations after affinity changes", func() {
		// TODO: Re-enable the spec. On the 3-node dev cluster the anti-affinity
		// reshuffle transiently leaves a VM Unschedulable ("didn't match pod
		// anti-affinity rules"), which trips the framework fail-fast rule.
		...
		By("Creating vm-a", func() {
			...
			vmAObs = vmobs.StartObserver(ctx, f, vmA)
		...
		By("Creating vm-b, vm-c and vm-d", func() {
			...
			vmBObs := vmobs.StartObserver(ctx, f, vmB)
			vmCObs = vmobs.StartObserver(ctx, f, vmC)
			vmDObs := vmobs.StartObserver(ctx, f, vmD)
			err = vmBObs.WaitFor(vmobs.BeRunning(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
			err = vmCObs.WaitFor(vmobs.BeRunning(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
			err = vmDObs.WaitFor(vmobs.BeRunning(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		By("Changing vm-c affinity to anti-affinity and verifying migration to another node", func() {
			err := vmCObs.WaitFor(migratableConditionTrue(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
	...
	It("keeps placement when nodeSelector points to the current node and migrates after switching it to another node", func() {
		...
		var vmNodeSelectorObs vmobs.Observer
		By("Creating the virtual machine", func() {
			...
			vmNodeSelectorObs = vmobs.StartObserver(ctx, f, vmNodeSelector)
			err = vmNodeSelectorObs.WaitFor(vmobs.BeRunning(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
			err = vmNodeSelectorObs.WaitFor(migratableConditionTrue(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		By("Setting spec.nodeSelector to another worker node and verifying migration", func() {
			...
			waitForStabilizedVMMigration(
				...
				targetNode,
				migrationTargetMustMatch,
				// v1.10 keeps Migratable=True on a machine pinned to the node it
				// runs on; the NoMigrationTarget reason exists only on newer releases.
				migratableConditionTrue(),
				framework.MaxTimeout,
			)
		})
	...
	It("keeps placement when nodeAffinity points to the current node and migrates after switching it to another node", func() {
		// TODO: Re-enable the scenario. The volume migration of the local root disk
		// fails: the migration target attachment pod is created before the volume
		// swap lands in the KVVM spec, can never schedule (the PV is node-local) and
		// the target virt-launcher times out waiting for the domain.
		...
		var vmNodeAffinityObs vmobs.Observer
		By("Creating the virtual machine", func() {
			err := f.CreateWithDeferredDeletion(
				ctx,
				object.NewVDFromCVI(
			...
			vmNodeAffinityObs = vmobs.StartObserver(ctx, f, vmNodeAffinity)
			err = vmNodeAffinityObs.WaitFor(vmobs.BeRunning(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
			err = vmNodeAffinityObs.WaitFor(migratableConditionTrue(), framework.LongTimeout)
			Expect(err).NotTo(HaveOccurred())
		})
...
// freshMigrationSucceededAfter reports a migration started at or after
// notBefore has finished successfully and the VM status has settled on the
// migration target node. A Failed result is a definite error.
func freshMigrationSucceededAfter(notBefore time.Time) vmobs.Predicate {
	return func(m *v1alpha2.VirtualMachine) (bool, error) {
		state := m.Status.MigrationState
		if state == nil || state.StartTimestamp == nil || state.StartTimestamp.UTC().Before(notBefore) {
			return false, nil
		}
		if state.EndTimestamp.IsZero() {
			return false, nil
		}
		if state.Result == v1alpha2.MigrationResultFailed {
			return false, fmt.Errorf("migration failed for vm %s/%s: %s", m.Namespace, m.Name, migrationFailureDetails(m))
		}
		if state.Result != v1alpha2.MigrationResultSucceeded {
			return false, nil
		}
		// Wait for status.node to settle on the migration target.
		return m.Status.Node == state.Target.Node, nil
	}
}
...
func waitForStabilizedVMMigration(
	...
	err := obs.WaitFor(freshMigrationSucceededAfter(notBefore), timeout)
	if err != nil {
		util.SkipIfKnownMigrationFailureWithContext(ctx, getVirtualMachine(ctx, f, key.Name))
	}
	Expect(err).NotTo(HaveOccurred())
...
// EXCEPTION: this is a bounded quiescence check — the VM must NOT migrate for
// the given window after a placement change that should be a no-op. An
// observer Never invariant cannot express it: the invariant would stay armed
// for the rest of the spec and misfire on the migration the same spec
// intentionally triggers later. Consistently is used deliberately here.
...
func updateVMPlacement(ctx context.Context, f *framework.Framework, name string, mutate func(*v1alpha2.VirtualMachine)) {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		mutate(vm)
		return f.GenericClient().Update(ctx, vm)
	})
	Expect(err).NotTo(HaveOccurred())
}
