...
var _ = DescribeTable("VirtualMachineCancelMigration", Label(label.SIGCompute), Label(precheck.NoPrecheck), func(bootloaderType v1alpha2.BootloaderType) {
	...
	// The abort is recorded by virt-handler: a running job is cancelled, a job
	// that has not been created yet is finalized as aborted right away. Under
	// parallel load the target preparation before either can take minutes.
...
func untilAbortStatusExists(ctx context.Context, timeout time.Duration, vm *v1alpha2.VirtualMachine) {
	...
	eventually.Until(func() error {
		...
		// Checked before AbortRequested: a migration that dies on its own instead of
		// being aborted is a definite failure, whatever the abort request says.
