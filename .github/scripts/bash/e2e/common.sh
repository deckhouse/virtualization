...
# Answers whether the text gate_accepted echoed is an admission answer about the
# gates and not a transport failure: the API server prints the rejection as
# 'admission webhook "..." denied the request: ...'. A transport failure never
# carries 'denied the request' - but it can mention the webhook, as in the
# 'failed calling admission webhook' of older apiservers, so only the former
# phrase is a safe marker.
# Usage: admission_denied <error_text>
admission_denied() {
  grep -q 'denied the request' <<< "$1"
}
