...
// +crd-enricher:deckhouse:documentation:examples={apiVersion: virtualization.deckhouse.io/v1alpha2, kind: VirtualMachine, metadata: {name: example-vm}, spec: {blockDeviceRefs: [{kind: VirtualDisk, name: example-disk}], cpu: {coreFraction: 10%, cores: 1}, memory: {size: 1Gi}, provisioning: {type: UserDataRef, userDataRef: {kind: Secret, name: cloudinit-for-vms}}, virtualMachineClassName: cpu-host-on-worker-nodes}}
...
type VirtualMachineSpec struct {
	...
	// +crd-enricher:raw:items.properties.maxSkew.description="MaxSkew describes the degree to which pods may be unevenly distributed.\nWhen `whenUnsatisfiable=DoNotSchedule`, it is the maximum permitted difference between the number of matching pods in the target topology and the global minimum.\nThe global minimum is the minimum number of matching pods in an eligible domain, or zero if the number of eligible domains is less than MinDomains.\n\nFor example, in a 3-zone cluster with MaxSkew set to 1 and pods with the same labelSelector spread as 2/2/1 across zone1, zone2, and zone3, the global minimum is 1:\n- if MaxSkew is 1, the incoming pod can only be scheduled to zone3 (becoming 2/2/2); scheduling it onto zone1 or zone2 would make the ActualSkew (3-1) violate MaxSkew (1);\n- if MaxSkew is 2, the incoming pod can be scheduled onto any zone.\n\nWhen `whenUnsatisfiable=ScheduleAnyway`, it is used to give higher precedence to topologies that satisfy it.\nIt is a required field. Default value is 1 and 0 is not allowed."
	// +crd-enricher:raw:items.properties.minDomains.description="MinDomains indicates a minimum number of eligible domains.\nWhen the number of eligible domains with matching topology keys is less than minDomains, Pod Topology Spread treats the global minimum as 0, and then the skew is calculated.\nWhen the number of eligible domains with matching topology keys equals or is greater than minDomains, this value has no effect on scheduling.\nAs a result, when the number of eligible domains is less than minDomains, the scheduler will not schedule more than maxSkew pods to those domains.\nIf the value is nil, the constraint behaves as if MinDomains is equal to 1.\nValid values are integers greater than 0.\nWhen the value is not nil, WhenUnsatisfiable must be DoNotSchedule.\n\nFor example, in a 3-zone cluster with MaxSkew set to 2, MinDomains set to 5, and pods with the same labelSelector spread as 2/2/2 across zone1, zone2, and zone3, the number of domains is less than 5 (MinDomains), so the global minimum is treated as 0.\nIn this situation, a new pod with the same labelSelector cannot be scheduled, because the computed skew would be 3 (3-0) if the new pod is scheduled to any of the three zones, which violates MaxSkew."
	// +crd-enricher:raw:items.properties.whenUnsatisfiable.description="WhenUnsatisfiable indicates how to deal with a pod if it does not satisfy the spread constraint.\n- DoNotSchedule (default) tells the scheduler not to schedule it.\n- ScheduleAnyway tells the scheduler to schedule the pod in any location, but giving higher precedence to topologies that would help reduce the skew.\n\nA constraint is considered Unsatisfiable for an incoming pod if and only if every possible node assignment for that pod would violate MaxSkew on some topology.\nFor example, in a 3-zone cluster with MaxSkew set to 1 and pods with the same labelSelector spread as 3/1/1 across zone1, zone2, and zone3, if WhenUnsatisfiable is DoNotSchedule, the incoming pod can only be scheduled to zone2 or zone3 (becoming 3/2/1 or 3/1/2), because ActualSkew (2-1) on zone2 or zone3 satisfies MaxSkew (1).\nIn other words, the cluster can still be imbalanced, but the scheduler will not make it more imbalanced.\nIt is a required field."
