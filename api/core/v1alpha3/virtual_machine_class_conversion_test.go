...
var _ = Describe("VirtualMachineClass Conversion", func() {
	...
	Context("Round-trip conversion", func() {
		DescribeTable("should preserve values through v2 -> v3 -> v2 conversion",
			func(v2CoreFractions []v1alpha2.CoreFractionValue) {
				originalV2 := &v1alpha2.VirtualMachineClass{
					ObjectMeta: metav1.ObjectMeta{Name: "test-class"},
					Spec: v1alpha2.VirtualMachineClassSpec{
						SizingPolicies: []v1alpha2.SizingPolicy{
							{
				...
				v3Class := &VirtualMachineClass{}
				err := v3Class.ConvertFrom(originalV2)
				Expect(err).NotTo(HaveOccurred())

				roundTripV2 := &v1alpha2.VirtualMachineClass{}
				err = v3Class.ConvertTo(roundTripV2)
				Expect(err).NotTo(HaveOccurred())
			...
			Entry("single value", []v1alpha2.CoreFractionValue{5}),
			Entry("multiple values", []v1alpha2.CoreFractionValue{5, 10, 25, 50, 100}),
			Entry("boundary values", []v1alpha2.CoreFractionValue{1, 100}),
		)
	})
