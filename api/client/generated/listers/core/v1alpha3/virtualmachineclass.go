...
type VirtualMachineClassLister interface {
	...
	List(selector labels.Selector) (ret []*corev1alpha3.VirtualMachineClass, err error)
	// Get retrieves the VirtualMachineClass from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*corev1alpha3.VirtualMachineClass, error)
...
type virtualMachineClassLister struct {
	listers.ResourceIndexer[*corev1alpha3.VirtualMachineClass]
}
...
func NewVirtualMachineClassLister(indexer cache.Indexer) VirtualMachineClassLister {
	return &virtualMachineClassLister{listers.New[*corev1alpha3.VirtualMachineClass](indexer, corev1alpha3.Resource("virtualmachineclass"))}
}
