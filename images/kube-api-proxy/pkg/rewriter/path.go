...
func (rw *RuleBasedRewriter) RewritePath(urlPath string) (*RewriteRequestResult, error) {
	// Is it a webhook?
	if webhookRule, ok := rw.Rules.Webhooks[urlPath]; ok {
		return &RewriteRequestResult{
			IsWebhook:        true,
			OrigGroup:        webhookRule.Group,
			OrigResourceType: webhookRule.Resource,
			TargetPath:       webhookRule.Path,
		}, nil
	}
	...
	if strings.HasPrefix(urlPath, "/apis/") || urlPath == "/apis" {
		...
		res, err := RewriteAPIPath(pathItems, rw.Rules)
		if err != nil {
			return nil, err
		}
		if res == nil {
...
// RewriteAPIPath rewrites GROUP and RESOURCETYPE in these API calls:
// - /apis/GROUP
// - /apis/GROUP/VERSION
...
func RewriteAPIPath(pathItems []string, rules *RewriteRules) (*RewriteRequestResult, error) {
