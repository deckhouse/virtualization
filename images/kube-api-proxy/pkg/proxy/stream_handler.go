...
func (s *streamRewriter) start(ctx context.Context) {
	...
	for {
		...
		if res != &got {
			...
			s.metrics.HandledRequestsError()
...
func (s *streamRewriter) transformWatchEvent(ev *metav1.WatchEvent) (*metav1.WatchEvent, error) {
	...
	if err != nil {
		if errors.Is(err, rewriter.SkipItem) {
			s.metrics.RewriteSuccess()
			return nil, err
		}
		s.metrics.RewriteError()
...
func (s *streamRewriter) writeEvent(ev *metav1.WatchEvent) {
	...
	bytesCounterWriter := BytesCounterWriterWrap(s.dst)
	_, err = io.Copy(bytesCounterWriter, io.NopCloser(bytes.NewBuffer(rwrEventBytes)))
	if err != nil {
	...
	s.metrics.ToClientBytesAdd(CounterValue(bytesCounterWriter))
