...
var _ = Describe("ReadyHandler", func() {
	DescribeTable("Handle",
		func(slices []client.Object, expectedReason string, expectedStatus metav1.ConditionStatus) {
			...
			node := &v1alpha2.NodeUSBDevice{
				...
				Status: v1alpha2.NodeUSBDeviceStatus{
					Attributes: v1alpha2.NodeUSBDeviceAttributes{Name: "usb-device-1"},
					NodeName:   "node-a",
				},
			}
		...
		Entry("device found", []client.Object{&resourcev1.ResourceSlice{
		...
		Entry("device not found", nil, string(nodeusbdevicecondition.NotFound), metav1.ConditionFalse),
	)
})
