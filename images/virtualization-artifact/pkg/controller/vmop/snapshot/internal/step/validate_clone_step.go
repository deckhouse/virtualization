...
type ValidateCloneStep struct {
	client   client.Client
	recorder eventrecord.EventRecorderLogger
	cb       *conditions.ConditionBuilder
}

func NewValidateCloneStep(
	client client.Client,
	recorder eventrecord.EventRecorderLogger,
	cb *conditions.ConditionBuilder,
) *ValidateCloneStep {
	return &ValidateCloneStep{
		client:   client,
		recorder: recorder,
		cb:       cb,
	}
...
func (s ValidateCloneStep) Take(ctx context.Context, vmop *v1alpha2.VirtualMachineOperation) (*reconcile.Result, error) {
	if c, ok := conditions.GetCondition(s.cb.GetType(), vmop.Status.Conditions); ok && c.Status == metav1.ConditionTrue {
		return nil, nil
	}
