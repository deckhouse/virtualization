...
var _ = Describe("WaitingDisksReadyStep", func() {
	...
	Describe("Disk ownership check with two disks", func() {
		// Setup: vmop has two disks in resources:
		// - "our-disk" belongs to this vmop (UID matches)
		// - "other-disk" belongs to a different vmop (UID doesn't match)

		It("should wait when our disk is not ready (both disks not ready)", func() {
			vmop := createCloneVMOP("default", "test-vmop", "test-vm", "test-snapshot")
			vmop.Status.Resources = []v1alpha2.SnapshotResourceStatus{
				{Kind: v1alpha2.VirtualDiskKind, Name: "our-disk"},
				{Kind: v1alpha2.VirtualDiskKind, Name: "other-disk"},
			}

			ourDisk := createVirtualDisk("default", "our-disk", "test-vmop-uid", v1alpha2.DiskProvisioning)
			otherDisk := createVirtualDisk("default", "other-disk", "different-vmop-uid", v1alpha2.DiskProvisioning)

			var err error
			fakeClient, err = testutil.NewFakeClientWithObjects(vmop, ourDisk, otherDisk)
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(result).NotTo(BeNil(), "should wait because our disk is not ready")
		})

		It("should wait when our disk is not ready (other disk is ready)", func() {
			vmop := createCloneVMOP("default", "test-vmop", "test-vm", "test-snapshot")
			vmop.Status.Resources = []v1alpha2.SnapshotResourceStatus{
				{Kind: v1alpha2.VirtualDiskKind, Name: "our-disk"},
				{Kind: v1alpha2.VirtualDiskKind, Name: "other-disk"},
			...
			ourDisk := createVirtualDisk("default", "our-disk", "test-vmop-uid", v1alpha2.DiskProvisioning)
			otherDisk := createVirtualDisk("default", "other-disk", "different-vmop-uid", v1alpha2.DiskReady)
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(result).NotTo(BeNil(), "should wait because our disk is not ready, regardless of other disk status")
		})

		It("should proceed when our disk is ready (other disk is not ready)", func() {
			vmop := createCloneVMOP("default", "test-vmop", "test-vm", "test-snapshot")
			vmop.Status.Resources = []v1alpha2.SnapshotResourceStatus{
				{Kind: v1alpha2.VirtualDiskKind, Name: "our-disk"},
				{Kind: v1alpha2.VirtualDiskKind, Name: "other-disk"},
			...
			ourDisk := createVirtualDisk("default", "our-disk", "test-vmop-uid", v1alpha2.DiskReady)
			otherDisk := createVirtualDisk("default", "other-disk", "different-vmop-uid", v1alpha2.DiskProvisioning)
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(BeNil(), "should proceed because our disk is ready, we don't wait for other vmop's disks")
		})
	})
})
