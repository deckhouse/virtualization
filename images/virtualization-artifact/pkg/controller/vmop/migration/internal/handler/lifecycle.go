...
func getMessageByMigrationFailedReason(mig *virtv1.VirtualMachineInstanceMigration) string {
	cond, found := conditions.GetKVVMIMCondition(VirtualMachineInstanceMigrationFailed, mig.Status.Conditions)

	if cond.Status == corev1.ConditionTrue && found {
		switch cond.Reason {
		case VirtualMachineInstanceMigrationFailedReasonVMIDoesNotExist, VirtualMachineInstanceMigrationFailedReasonVMIIsShutdown:
...
const (
	VirtualMachineInstanceMigrationFailed virtv1.VirtualMachineInstanceMigrationConditionType = "migrationFailed"
)
