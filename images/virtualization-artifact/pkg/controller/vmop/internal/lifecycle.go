...
func (h LifecycleHandler) canBeRun(ctx context.Context, vmop *virtv2.VirtualMachineOperation) (bool, error) {
	...
	for _, other := range vmopList.Items {
		...
		if !operationIsInProgress(&other) {
			return false, nil
		}
	}

	return oldest != nil && oldest.GetUID() == vmop.GetUID(), nil
}

func operationIsInProgress(vmop *virtv2.VirtualMachineOperation) bool {
