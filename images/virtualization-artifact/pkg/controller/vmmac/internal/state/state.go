...
func (s *state) VirtualMachineMACLease(ctx context.Context) (*virtv2.VirtualMachineMACAddressLease, error) {
	...
	if s.lease == nil {
		...
		for i, lease := range leases.Items {
			boundCondition, exist := conditions.GetCondition(vmmaclcondition.BoundType, lease.Status.Conditions)
...
func (s *state) VirtualMachine(ctx context.Context) (*virtv2.VirtualMachine, error) {
	...
	if s.mac.Status.VirtualMachine != "" {
		...
		if vm.Status.VirtualMachineMACAddress == s.mac.Name && vm.Status.MACAddress == s.mac.Status.Address {
			s.vm = vm
		}
	}
	...
	if s.vm == nil {
		...
		for i, vm := range vms.Items {
			if vm.Spec.VirtualMachineMACAddress == s.mac.Name || vm.Spec.VirtualMachineMACAddress == "" && vm.Name == netmanager.GetVirtualMachineNameFromVMMAC(s.mac) {
				s.vm = &vms.Items[i]
				break
			}
		}
