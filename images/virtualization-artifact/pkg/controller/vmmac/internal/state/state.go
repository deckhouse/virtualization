...
func (s *state) VirtualMachineMACLease(ctx context.Context) (*virtv2.VirtualMachineMACAddressLease, error) {
	...
	leaseName := mac.MACAddressToLeaseName(s.mac.Status.Address)
...
func (s *state) VirtualMachine(ctx context.Context) (*virtv2.VirtualMachine, error) {
	...
	if s.vm == nil {
		...
		for i, vm := range vms.Items {
			if vm.Spec.VirtualMachineIPAddress == s.mac.Name || vm.Spec.VirtualMachineIPAddress == "" && vm.Name == netmanage.GetVirtualMachineNameFromVMMAC(s.mac) {
				s.vm = &vms.Items[i]
				break
			}
		}
