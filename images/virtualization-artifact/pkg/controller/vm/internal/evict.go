...
func (h *EvictHandler) Handle(ctx context.Context, s state.VirtualMachineState) (reconcile.Result, error) {
	...
	if kvvmi.Status.EvacuationNodeName == "" {
		conditions.RemoveCondition(vmcondition.TypeNeedEvict, &changed.Status.Conditions)
		return reconcile.Result{}, nil
	}

	conditions.SetCondition(
		conditions.NewConditionBuilder(vmcondition.TypeNeedEvict).
			Generation(changed.GetGeneration()).
			Status(metav1.ConditionTrue).
			Reason(vmcondition.TypeNeedEvict).
