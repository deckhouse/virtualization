...
var _ = Describe("TestEvictHandler", func() {
	...
	newVM := func(withCond bool) *virtv2.VirtualMachine {
		...
		if withCond {
			vm.Status.Conditions = append(vm.Status.Conditions, metav1.Condition{
				...
				Reason:  vmcondition.ReasonNeedEvict.String(),
	...
	DescribeTable("Condition NeedEvict should be in expected state",
		func(vm *virtv2.VirtualMachine, kvvmi *virtv1.VirtualMachineInstance, condShouldExists bool, expectedStatus metav1.ConditionStatus, expectedReason vmcondition.Reason) {
			...
			needEvict, exists := conditions.GetCondition(vmcondition.TypeNeedEvict, newVM.Status.Conditions)
			if condShouldExists {
		...
		Entry("Should be added cond NeedEvict with true", newVM(false), newKVVMI("node1"), true, metav1.ConditionTrue, vmcondition.ReasonNeedEvict),
		Entry("Should be removed cond NeedEvict", newVM(true), newKVVMI(""), false, metav1.ConditionStatus(""), vmcondition.Reason("")),
		Entry("Should be do nothing", newVM(true), newKVVMI("node1"), true, metav1.ConditionTrue, vmcondition.ReasonNeedEvict),
	)
})
