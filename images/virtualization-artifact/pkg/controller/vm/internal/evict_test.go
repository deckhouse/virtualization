...
var _ = Describe("TestEvictHandler", func() {
	...
	DescribeTable("Condition NeedEvict should be in expected state",
		...
		Entry("Should be added cond NeedEvict with true", newVM(false), newKVVMI("node1"), true, metav1.ConditionTrue, vmcondition.ReasonNeedsEvict),
		Entry("Should be removed cond NeedEvict", newVM(true), newKVVMI(""), false, metav1.ConditionStatus(""), vmcondition.Reason("")),
		Entry("Should be do nothing", newVM(true), newKVVMI("node1"), true, metav1.ConditionTrue, vmcondition.ReasonNeedsEvict),
	)
})
