...
type FirmwareHandler struct {
	...
	firmwareMinSupportedVersion string
}
...
func (f FirmwareHandler) Handle(ctx context.Context, s state.VirtualMachineState) (reconcile.Result, error) {
	...
	if kvvmi == nil {
		...
		changed.Status.FirmwareVersion = f.firmwareVersion
		f.removeCondition(changed)
		return reconcile.Result{}, nil
	}

	if f.needUpdate(changed.Status.FirmwareVersion) {
...
func (f FirmwareHandler) needUpdate(currentVersion string) bool {
