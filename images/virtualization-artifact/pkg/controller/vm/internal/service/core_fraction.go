...
// cover it (kept below 100% to keep the pod Burstable) and snaps it to the sizing
...
func (s *CoreFractionService) Calculate(vm *v1alpha2.VirtualMachine, class *v1alpha2.VirtualMachineClass, rec Recommendation) (Decision, error) {
	...
	steps := sizingpolicy.AutoCoreFractions(class, cores)
	// With no policy (or a policy that lists no fractions) the steps are the default grid,
	// which tops out at 99%. A policy contributes its own steps without 100%, so desired
	// never leaves what the policy allows and holds at its top step.
	desired := sizingpolicy.QuantizeCoreFractionUp(raw, steps)

	d := Decision{CurrentCoreFraction: current, DesiredCoreFraction: desired}
	switch {
