...
func GetOriginalDiskName(prefixedName string) (string, v1alpha2.BlockDeviceKind) {
	...
	case strings.HasPrefix(prefixedName, VMDDiskPrefix):
		return strings.TrimPrefix(prefixedName, VMDDiskPrefix), v1alpha2.DiskDevice
	case strings.HasPrefix(prefixedName, VMIDiskPrefix):
		return strings.TrimPrefix(prefixedName, VMIDiskPrefix), v1alpha2.ImageDevice
	case strings.HasPrefix(prefixedName, CVMIDiskPrefix):
		return strings.TrimPrefix(prefixedName, CVMIDiskPrefix), v1alpha2.ClusterImageDevice
	}
...
func ApplyVirtualMachineSpec(
	...
	setNetwork(kvvm, vm.Spec, clusterUUID)
...
func setNetwork(kvvm *KVVM, vmSpec v1alpha2.VirtualMachineSpec, clusterUUID string) {
	...
	for _, n := range network.CreateNetworkSpec(vmSpec) {
		kvvm.SetNetworkInterface(n.InterfaceName, generateMACAddress(clusterUUID))
	}
}

func generateMACAddress(clusterUUID string) string {
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	return fmt.Sprintf("%s:%02x:%02x:%02x", mac.GenerateOUI(clusterUUID), r.Intn(256), r.Intn(256), r.Intn(256))
}

func setNetworksAnnotation(kvvm *KVVM, vmSpec v1alpha2.VirtualMachineSpec) error {
