...
import (
	...
	commonvmop "github.com/deckhouse/virtualization-controller/pkg/common/vmsop"
...
func SetupGC(mgr manager.Manager, log *log.Logger, gcSettings config.BaseGcSettings) error {
	vmopGCMgr := newVMSOPGCManager(mgr.GetClient(), gcSettings.TTL.Duration, 10)
...
func newVMSOPGCManager(client client.Client, ttl time.Duration, max int) *vmopGCManager {
	if ttl == 0 {
		ttl = 24 * time.Hour
	}
	if max == 0 {
...
func (m *vmopGCManager) getIndex(obj client.Object) string {
	vmop, ok := obj.(*v1alpha2.VirtualMachineSnapshotOperation)
	if !ok {
		return ""
	}
