...
func (h DeletionHandler) Handle(ctx context.Context, vmop *v1alpha2.VirtualMachineSnapshotOperation) (reconcile.Result, error) {
	log := logger.FromContext(ctx)

	// Add finalizer for operations in progress
	if vmop.DeletionTimestamp.IsZero() {
		if vmop.Status.Phase == v1alpha2.VMSOPPhaseInProgress {
			log.Debug("Add cleanup finalizer while in the InProgress phase")
			controllerutil.AddFinalizer(vmop, v1alpha2.FinalizerVMSOPCleanup)
		}
	...
	if vm == nil {
		controllerutil.RemoveFinalizer(vmop, v1alpha2.FinalizerVMSOPCleanup)
		return reconcile.Result{}, nil
	}
	...
	if vmop.DeletionTimestamp.IsZero() {
		log.Debug("Remove cleanup finalizer from VirtualMachineSnapshotOperation: not InProgress state", "phase", vmop.Status.Phase)
	} else {
		log.Info("Deletion observed: remove cleanup finalizer from VirtualMachineSnapshotOperation", "phase", vmop.Status.Phase)
	}
	controllerutil.RemoveFinalizer(vmop, v1alpha2.FinalizerVMSOPCleanup)
