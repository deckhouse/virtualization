...
func (s WaitForDVStep) Take(ctx context.Context, vi *virtv2.VirtualImage) (*reconcile.Result, error) {
	...
	vi.Status.Progress = s.diskService.GetProgress(s.dv, vi.Status.Progress, service.NewScaleOption(0, 100))
...
func (s WaitForDVStep) checkQoutaNotExceededCondition(vi *virtv2.VirtualImage) (ok bool) {
	dvQuotaNotExceededCondition := conditions.GetDataVolumeCondition(conditions.DataVolumeConditionTypeQoutaNotExceeded, s.dv.Status.Conditions)
	if dvQuotaNotExceededCondition != nil && dvQuotaNotExceededCondition.Status == corev1.ConditionFalse {
		vi.Status.Phase = virtv2.ImagePending
		s.cb.
			Status(metav1.ConditionFalse).
...
func (s WaitForDVStep) checkRunningCondition(vi *virtv2.VirtualImage) (ok bool) {
	dvRunningCondition := conditions.GetDataVolumeCondition(conditions.DataVolumeConditionTypeRunning, s.dv.Status.Conditions)
	if dvRunningCondition != nil {
		if dvRunningCondition.Reason == conditions.DataVolumeConditionReasonImagePullFailed {
			vi.Status.Phase = virtv2.ImagePending
			s.cb.
				Status(metav1.ConditionFalse).
		...
		if strings.Contains(dvRunningCondition.Reason, "Error") {
			vi.Status.Phase = virtv2.ImagePending
			s.cb.
				Status(metav1.ConditionFalse).
