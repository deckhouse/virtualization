...
const imageMonitorControllerName = "vi-image-monitor-controller"
...
func SetupImageMonitorGC(
	...
	source, err := gc.NewCronSource(monitorSettings.Schedule, imageMon, log.With("resource", "vi"))
	if err != nil {
		return err
	}
...
func (m *imageMonitorManager) ShouldBeDeleted(obj client.Object) bool {
	...
	// The actual status update happens in ListForDelete
	_ = vi
	return false
}
...
func (m *imageMonitorManager) ListForDelete(ctx context.Context, now time.Time) ([]client.Object, error) {
	...
	for i := range viList.Items {
		...
		// Only check Ready images with DVCR storage
		...
		if !exists {
			...
			if err := m.markImageLost(ctx, vi, registryURL); err != nil {
				// Log but continue
				continue
			}
...
func (m *imageMonitorManager) markImageLost(ctx context.Context, vi *v1alpha2.VirtualImage, registryURL string) error {
	...
	return m.client.Status().Update(ctx, vi)
}
