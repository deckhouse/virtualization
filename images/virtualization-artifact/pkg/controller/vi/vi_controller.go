...
func NewController(
	...
	importer := service.NewImporterService(dvcr, mgr.GetClient(), importerImage, imagePullSecret, requirements, PodPullPolicy, PodVerbose, ControllerName, protection)
	uploader := service.NewUploaderService(dvcr, mgr.GetClient(), uploaderImage, imagePullSecret, requirements, PodPullPolicy, PodVerbose, ControllerName, protection)
	bounder := service.NewBounderPodService(dvcr, mgr.GetClient(), bounderImage, imagePullSecret, requirements, PodPullPolicy, PodVerbose, ControllerName, protection)
	...
	err = reconciler.SetupController(ctx, mgr, viController, log)
	if err != nil {
		return nil, err
	}
