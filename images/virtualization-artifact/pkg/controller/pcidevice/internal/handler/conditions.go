...
func setReadyCondition(
	...
	reason pcidevicecondition.ReadyReason,
	message string,
	lastTransitionTime *metav1.Time,
) {
	cb := conditions.NewConditionBuilder(pcidevicecondition.ReadyType).
		Generation(pciDevice.GetGeneration()).
		Status(status).
		Reason(reason).
		Message(message)
...
func setAttachedCondition(
	...
	target *[]metav1.Condition,
	status metav1.ConditionStatus,
	reason pcidevicecondition.AttachedReason,
	message string,
) {
	cb := conditions.NewConditionBuilder(pcidevicecondition.AttachedType).
		Generation(pciDevice.GetGeneration()).
		Status(status).
		Reason(reason).
		Message(message)
