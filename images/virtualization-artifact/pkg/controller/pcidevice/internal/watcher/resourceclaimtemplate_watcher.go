	...
	return ctr.Watch(
		source.Kind(mgr.GetCache(),
			...
			handler.TypedEnqueueRequestsFromMapFunc(func(_ context.Context, template *resourcev1.ResourceClaimTemplate) []reconcile.Request {
				name, ok := mapResourceClaimTemplateToPCIDeviceName(template)
				if !ok {
					return nil
				}
...
func mapResourceClaimTemplateToPCIDeviceName(template *resourcev1.ResourceClaimTemplate) (string, bool) {
	if template == nil {
		return "", false
	}

	for _, ref := range template.OwnerReferences {
		if ref.Kind == v1alpha2.PCIDeviceKind && ref.APIVersion == v1alpha2.SchemeGroupVersion.String() {
