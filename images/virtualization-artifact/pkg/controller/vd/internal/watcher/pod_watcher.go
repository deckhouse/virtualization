...
func (w PodWatcher) filterUpdateEvents(e event.UpdateEvent) bool {
	...
	oldPodScheduled, _ := conditions.GetPodCondition(corev1.PodScheduled, oldPod.Status.Conditions)
	newPodScheduled, _ := conditions.GetPodCondition(corev1.PodScheduled, newPod.Status.Conditions)

	return oldPodScheduled.LastTransitionTime != newPodScheduled.LastTransitionTime
}
