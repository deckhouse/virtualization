...
func (s CreateDataVolumeFromVirtualImageStep) Take(ctx context.Context, vd *virtv2.VirtualDisk) (*reconcile.Result, error) {
	...
	source, err := getSource(vd, viRef)
	if err != nil {
		return nil, fmt.Errorf("get source %q: %w", viRefKey, err)
	}

	size, err := getPVCSize(vd, viRef)
	if err != nil {
		// TODO: setPhaseConditionToFailed(cb, &vd.Status.Phase, err)
...
func getPVCSize(vd *virtv2.VirtualDisk, viRef *virtv2.VirtualImage) (resource.Quantity, error) {
...
func getSource(vd *virtv2.VirtualDisk, viRef *virtv2.VirtualImage) (*cdiv1.DataVolumeSource, error) {
