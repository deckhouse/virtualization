...
func (h MigrationHandler) getAction(ctx context.Context, vd *virtv2.VirtualDisk) (action, error) {
	...
	if inUse.Reason != vdcondition.AttachedToVirtualMachine.String() {
		return none, nil
	}
...
func (h MigrationHandler) handleMigrate(ctx context.Context, vd *virtv2.VirtualDisk) error {
	...
	pvc, err := h.createMigrationPersistentVolumeClaim(ctx, vd, targetStorageClass, size)
	if err != nil {
		return err
	}
...
func (h MigrationHandler) createMigrationPersistentVolumeClaim(ctx context.Context, vd *virtv2.VirtualDisk, sc *storev1.StorageClass, size resource.Quantity) (*corev1.PersistentVolumeClaim, error) {
	...
	case 2:
		for _, pvc := range pvcs {
			...
			if pvc.Name == vd.Status.MigrationInfo.TargetPVC || pvc.Name != vd.Status.MigrationInfo.SourcePVC {
				return &pvc, nil
			}
		}
	default:
