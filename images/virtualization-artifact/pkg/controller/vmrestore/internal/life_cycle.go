...
func (h LifeCycleHandler) Handle(ctx context.Context, vmRestore *virtv2.VirtualMachineRestore) (reconcile.Result, error) {
	...
	vmbdas, err := h.getVirtualMachineBlockDeviceAttachments(ctx, restorerSecret)
	switch {
	case err == nil:
...
var (
	...
	ErrImageNotFound               = errors.New("not found")
)
...
func (h LifeCycleHandler) getVirtualMachineBlockDeviceAttachments(ctx context.Context, secret *corev1.Secret) ([]*virtv2.VirtualMachineBlockDeviceAttachment, error) {
	vmbdas, err := h.restorer.RestoreVirtualMachineBlockDeviceAttachments(ctx, secret)
	if err != nil {
		return nil, err
	}

	validatedVMBDAs := make([]*virtv2.VirtualMachineBlockDeviceAttachment, 0, len(vmbdas))

	for _, vmbda := range vmbdas {
		bdKind := vmbda.Spec.BlockDeviceRef.Kind
		bdName := vmbda.Spec.BlockDeviceRef.Name

		switch bdKind {
		case virtv2.VMBDAObjectRefKindClusterVirtualImage:
			cviKey := types.NamespacedName{Name: bdName, Namespace: vmbda.Namespace}
			cvi, err := object.FetchObject(ctx, cviKey, h.client, &virtv2.ClusterVirtualImage{})
			if err != nil {
				return nil, fmt.Errorf("failed to fetch the `ClusterVirtualImage`: %w", err)
			}

			if cvi == nil {
				return nil, fmt.Errorf("the `ClusterVirtualImage` %q %w", bdName, ErrImageNotFound)
			}
			validatedVMBDAs = append(validatedVMBDAs, vmbda)
		case virtv2.VMBDAObjectRefKindVirtualImage:
			viKey := types.NamespacedName{Name: bdName, Namespace: vmbda.Namespace}
			vi, err := object.FetchObject(ctx, viKey, h.client, &virtv2.VirtualImage{})
			if err != nil {
				return nil, fmt.Errorf("failed to fetch the `VirtualImage`: %w", err)
			}

			if vi == nil {
				return nil, fmt.Errorf("the `VirtualImage` %q %w", bdName, ErrImageNotFound)
			}
			validatedVMBDAs = append(validatedVMBDAs, vmbda)
		default:
			validatedVMBDAs = append(validatedVMBDAs, vmbda)
	...
	if len(validatedVMBDAs) != len(vmbdas) {
		return nil, fmt.Errorf("failed to extract all `VirtualMachineBlockDeviceAttachments` from the `VirtualMachineSnapshot`")
	}

	return validatedVMBDAs, nil
}
