...
func (v *VirtualDiskOverrideValidator) ProcessWithForce(ctx context.Context, vmRestoreUID string) error {
	vdKey := types.NamespacedName{Namespace: v.vd.Namespace, Name: v.vd.Name}
	...
	if vdObj != nil {
		if v, ok := vdObj.Annotations[annotations.AnnVMRestore]; ok && v == vmRestoreUID {
			return nil
		}
...
func (v *VirtualDiskOverrideValidator) AnnotateObject(vmRestoreUID string) {
	if v.vd.Annotations != nil {
		v.vd.Annotations[annotations.AnnVMRestore] = vmRestoreUID
	} else {
		v.vd.Annotations = make(map[string]string)
		v.vd.Annotations[annotations.AnnVMRestore] = vmRestoreUID
	}
}
