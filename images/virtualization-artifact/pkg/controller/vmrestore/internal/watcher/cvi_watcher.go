...
func NewClusterVirtualImageWatcher(client client.Client, restorer vmrestore.Restorer) *ClusterVirtualImageWatcher {
...
func (w ClusterVirtualImageWatcher) enqueueRequests(ctx context.Context, obj client.Object) (requests []reconcile.Request) {
	...
	for _, vmRestore := range vmRestores.Items {
		...
		for _, eVmbda := range vmbdas {
			if w.isViNameMatch(cvi.Name, eVmbda.Spec.BlockDeviceRef, vmRestore.Spec.NameReplacements) {
...
func (w ClusterVirtualImageWatcher) isViNameMatch(cviName string, bdr virtv2.VMBDAObjectRef, nameReplacements []virtv2.NameReplacement) bool {
	if bdr.Kind != virtv2.VMBDAObjectRefKindClusterVirtualImage {
		return false
	}
	...
	isNameMatch = cviName == bdr.Name

	for _, nr := range nameReplacements {
		if nr.From.Kind != virtv2.ClusterVirtualImageKind {
			continue
		}

		if nr.From.Name == cviName {
