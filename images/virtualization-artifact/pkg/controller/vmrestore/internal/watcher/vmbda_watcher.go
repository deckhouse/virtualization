	...
	return ctr.Watch(
		...
		predicate.Funcs{
			...
			UpdateFunc: func(e event.UpdateEvent) bool {
				// oldVMBDA, ok := e.ObjectOld.(*virtv2.VirtualMachineBlockDeviceAttachment)
				// if !ok {
				// 	return false
				// }
				// newVMBDA, ok := e.ObjectNew.(*virtv2.VirtualMachineBlockDeviceAttachment)
				// if !ok {
				// 	return false
				// }
				// oldPhase := oldVMBDA.Status.Phase
				// newPhase := newVMBDA.Status.Phase
