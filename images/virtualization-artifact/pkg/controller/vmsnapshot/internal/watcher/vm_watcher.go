...
func (w VirtualMachineWatcher) filterUpdateEvents(e event.UpdateEvent) bool {
	...
	oldFSReady, _ := conditions.GetConditionByType(vmcondition.TypeFilesystemReady.String(), oldVM.Status.Conditions)
	newFSReady, _ := conditions.GetConditionByType(vmcondition.TypeFilesystemReady.String(), newVM.Status.Conditions)

	if oldFSReady.Reason != newFSReady.Reason {
		return true
	}

	oldSnapshotting, _ := conditions.GetConditionByType(vmcondition.TypeSnapshotting.String(), oldVM.Status.Conditions)
	newSnapshotting, _ := conditions.GetConditionByType(vmcondition.TypeSnapshotting.String(), newVM.Status.Conditions)
