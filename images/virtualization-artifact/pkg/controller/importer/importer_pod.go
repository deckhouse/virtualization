...
func (imp *Importer) addVolumes(pod *corev1.Pod, container *corev1.Container) {
	...
	if imp.EnvSettings.DestinationTokenAuth {
		...
		podutil.AddVolume(pod, container,
			...
			podutil.CreateVolumeMount(destinationTokenVol, common.ImporterDestinationTokenDir),
			corev1.EnvVar{
				Name:  common.ImporterDestinationTokenFileVar,
