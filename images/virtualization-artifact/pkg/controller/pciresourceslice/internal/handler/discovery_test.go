...
import (
	...
	resourcest "github.com/deckhouse/virtualization-controller/pkg/controller/pciresourceslice/internal/state"
...
var _ = Describe("DiscoveryHandler", func() {
	DescribeTable("Handle",
		func(existingName, expectVendor string) {
			...
			builder := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(&v1alpha2.NodePCIDevice{})
			if existingName != "" {
				builder = builder.WithObjects(&v1alpha2.NodePCIDevice{
					ObjectMeta: metav1.ObjectMeta{Name: existingName},
					Status:     v1alpha2.NodePCIDeviceStatus{Attributes: v1alpha2.NodePCIDeviceAttributes{Name: existingName}},
				})
			}
			...
			created := &v1alpha2.NodePCIDevice{}
			Expect(cl.Get(context.Background(), types.NamespacedName{Name: "pci-device-1"}, created)).To(Succeed())
			if expectVendor != "" {
				Expect(created.Status.Attributes.VendorID).To(Equal(expectVendor))
			}
		},
		Entry("creates missing device", "", "10de"),
		Entry("syncs existing device attributes", "pci-device-1", "10de"),
	)
	...
	It("should skip when current ResourceSlice is absent", func() {
		...
		cl := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(&v1alpha2.NodePCIDevice{}).Build()
		h := NewDiscoveryHandler(cl)
		...
		created := &v1alpha2.NodePCIDevice{}
		err = cl.Get(context.Background(), types.NamespacedName{Name: "pci-device-1"}, created)
