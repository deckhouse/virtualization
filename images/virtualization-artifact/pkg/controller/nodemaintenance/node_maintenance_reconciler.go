...
import (
	...
	"github.com/deckhouse/virtualization-controller/pkg/controller/nodemaintenance/internal/watcher"
...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err := node.Fetch(ctx); err != nil {
		return reconcile.Result{}, err
	}

	if node.IsEmpty() {
		logger.FromContext(ctx).Info("Reconcile observe an absent Node: it may be deleted")
	...
	rec.SetResourceUpdater(func(_ context.Context) error {
		// The dialogue lives in the annotations, patched by the handler: the node itself is not ours to update.
		return nil
	})
...
func (r *Reconciler) statusGetter(node *corev1.Node) corev1.NodeStatus {
	return node.Status
}
