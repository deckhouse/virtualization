...
func (u *USBIPD) handleConnection(conn net.Conn) error {
	...
	if opCommon.Version != protocol.Version {
		return fmt.Errorf("unsupported USBIP version: %d", opCommon.Version)
	}

	if opCommon.Status != protocol.OpStatusOk {
		return fmt.Errorf("request failed: %s", opCommon.Status.String())
	}

	switch opCommon.Code {
	case protocol.OpReqDevList:
		if err := u.handleDeviceList(conn); err != nil {
			return fmt.Errorf("failed to handle OpReqDevList: %w", err)
		}
	case protocol.OpReqImport:
		if err := u.handleImportRequest(conn); err != nil {
			return fmt.Errorf("failed to handle OpReqImport: %w", err)
		}
		return nil
	case protocol.OpReqExport:
		if err := u.handleExportRequest(conn); err != nil {
			return fmt.Errorf("failed to handle OpRepExport: %w", err)
		}
	case protocol.OpReqUnexport:
		if err := u.handleUnexportRequest(conn); err != nil {
			return fmt.Errorf("failed to handle OpReqUnexport: %w", err)
		}
	case protocol.OpReqDevInfo, protocol.OpReqCrypkey:
		// nothing to do
	default:
		return fmt.Errorf("unsupported OpCommon.Code: %d", opCommon.Code)
	}
