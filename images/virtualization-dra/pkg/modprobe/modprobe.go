...
func KernelSupportsZst(release string) (bool, error) {
	parts := strings.Split(release, ".")
	if len(parts) < 2 {
		return false, fmt.Errorf("invalid release %s", release)
	}

	major, err := strconv.Atoi(parts[0])
	if err != nil {
		return false, fmt.Errorf("invalid major %s: %w", parts[0], err)
	}
	minor, err := strconv.Atoi(parts[1])
	if err != nil {
		return false, fmt.Errorf("invalid minor %s: %w", parts[1], err)
	}
