...
Initially copied from https://github.com/kubernetes/dynamic-resource-allocation/blob/v0.35.1/resourceslice/resourceslicecontroller.go#L177
*/

// TODO: https://github.com/kubernetes/kubernetes/issues/137011
...
type DroppedFieldsError struct {
	PoolName                  string
	SliceIndex                int
	DesiredSlice, ActualSlice *resourcev1.ResourceSlice
}
			...
			for _, device := range slice.Devices {
				for _, taint := range device.Taints {
					if taint.TimeAdded != nil {
						taint.TimeAdded.Time = taint.TimeAdded.Round(time.Second)
					}
				}
			}
		}
...
func newController(ctx context.Context, options Options) (*Controller, error) {
	...
	c := &Controller{
		...
		reconcileOnlyPoolName: options.ReconcileOnlyPoolName,
	}
	if c.queue == nil {
...
func (c *Controller) initInformer(ctx context.Context) error {
	...
	selector := fields.Set{
		resourcev1.ResourceSliceSelectorDriver:   c.driverName,
		resourcev1.ResourceSliceSelectorNodeName: "",
	}

	if c.owner != nil && c.owner.APIVersion == "v1" && c.owner.Kind == "Node" && c.reconcileOnlyPoolName == "" {
		selector[resourcev1.ResourceSliceSelectorNodeName] = c.owner.Name
	}
			...
			WatchFuncWithContext: func(ctx context.Context, options metav1.ListOptions) (watch.Interface, error) {
				...
				if c.reconcileOnlyPoolName != "" {
					return newWrapWatcher(ctx, w, func(event watch.Event) bool {
						resourceSlice, ok := event.Object.(*resourcev1.ResourceSlice)
						return ok && resourceSlice.Spec.Pool.Name == c.reconcileOnlyPoolName
					}), nil
				}
	...
	handler, err := informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new any) {
			oldSlice, ok := old.(*resourcev1.ResourceSlice)
			if !ok {
				return
			}
			newSlice, ok := new.(*resourcev1.ResourceSlice)
			if !ok {
				return
			}
...
func (c *Controller) syncPool(ctx context.Context, poolName string) error {
	...
	obsoleteSlices := make([]*resourcev1.ResourceSlice, 0, len(slices))
	...
	currentSlices := make([]*resourcev1.ResourceSlice, 0, len(slices))
	for _, slice := range slices {
		if slice.Spec.Pool.Generation < generation {
	...
	// a match or obsolete.
	currentSliceForDesiredSlice := make(map[int]*resourcev1.ResourceSlice, len(pool.Slices))
	...
	desiredPool := resourcev1.ResourcePool{
		Name:               poolName,
...
func (c *Controller) removeSlices(ctx context.Context, slices []*resourcev1.ResourceSlice) error {
	logger := klog.FromContext(ctx)
...
func (c *Controller) sliceStored(ctx context.Context, msg, poolName string, pool Pool, sliceIndex int, desiredSlice, actualSlice *resourcev1.ResourceSlice) {
...
func copyServerDefaults(desiredSlice, actualSlice *resourcev1.ResourceSlice) bool {
	copied := false
...
func sameSlice(existingSlice *resourcev1.ResourceSlice, desiredSlice *Slice) bool {
...
func copyTaintTimeAdded(from, to []resourcev1.Device) []resourcev1.Device {
	to = slices.Clone(to)
	for i, toDevice := range to {
		index := slices.IndexFunc(from, func(fromDevice resourcev1.Device) bool {
		...
		for j, toTaint := range toDevice.Taints {
			...
			index := slices.IndexFunc(fromDevice.Taints, func(fromTaint resourcev1.DeviceTaint) bool {
...
// tolerated.
func DevicesDeepEqual(a, b []resourcev1.Device) bool {
...
func deviceTaintEqual(a, b resourcev1.DeviceTaint) bool {
	if a.TimeAdded != nil && b.TimeAdded != nil {
		delta := b.TimeAdded.Sub(a.TimeAdded.Time)
