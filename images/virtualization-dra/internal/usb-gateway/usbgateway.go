...
func (c *USBGatewayController) getDevice(deviceName string) (*resourcev1beta1.Device, string, error) {
...
func (c *USBGatewayController) getVirtualizationDraResourceSlices() ([]resourcev1beta1.ResourceSlice, error) {
	slicesObj, err := c.resourceSliceIndexer.ByIndex(informer.DriverIndex, common.VirtualizationDraUSBDriverName)
	if err != nil {
		return nil, err
	}
...
func (c *USBGatewayController) getBusID(device *resourcev1beta1.Device) (string, error) {
	if device.Basic == nil {
		return "", fmt.Errorf("basic attribute is not exist")
	}
