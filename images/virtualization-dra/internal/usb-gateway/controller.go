...
func NewUSBGatewayController(
	...
	c := &USBGatewayController{
		...
		namespace:            namespace,
		nodeName:             nodeName,
		usbipdAddr:           net.JoinHostPort(usbipdHost, strconv.Itoa(usbipdPort)),
		client:               client,
...
func (c *USBGatewayController) queueAfterAdd(key string, duration time.Duration) {
...
func (c *USBGatewayController) createSecret() error {
	...
	_, err := c.client.CoreV1().Secrets(c.namespace).Create(context.Background(), secret, metav1.CreateOptions{})
	if err != nil {
