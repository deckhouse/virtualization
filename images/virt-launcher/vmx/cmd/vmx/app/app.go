...
VMX is a tool for gathering information from virtual machines running under the
...
Use the VMX command to retrieve essential data from virtual machines.
`

func NewVMXCommand() *cobra.Command {
	baseOpts := BaseOptions{}

	cmd := &cobra.Command{
		Use:           "vmx",
		Short:         "VMX - A tool to retrieve information from virtual machines managed by virt-launcher",
		Long:          vmxLong,
		SilenceUsage:  true,
		SilenceErrors: true,
