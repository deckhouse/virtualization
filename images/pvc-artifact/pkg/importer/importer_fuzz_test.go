...
// normal outcome; a panic, a hang, an unusable reader stack or a stream that
// cannot be drained is the finding.
func FuzzFormatReaders(f *testing.F) {
	...
	// iteration order of knownHeaders, so the reader stack must hold up either way.
	...
	// An ISO 9660 image. This module has no ISO detection, so the payload passes
	// through as an ordinary uncompressed stream.
	f.Add(isoImage())
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		// The reader stack the caller is handed has to be usable: processLayer
		// reads from the top reader without checking it.
		if fr.TopReader() == nil {
			t.Fatalf("a payload of %d bytes was accepted without a reader", len(data))
		}
