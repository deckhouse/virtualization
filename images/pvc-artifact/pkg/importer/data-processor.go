...
func (dp *DataProcessor) convert(url *url.URL) (ProcessingPhase, error) {
	...
	start := time.Now()
	klog.V(1).Infof("Convert to %s started at %s (qemu-img convert threads=%d)", format, start.Format(time.RFC3339Nano), dp.qemuConvertThreads)
	...
	klog.V(1).Infof("Convert to %s finished at %s (duration %s)", format, end.Format(time.RFC3339Nano), end.Sub(start))
