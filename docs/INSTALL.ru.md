...
1. Включите модуль `virtualization`:
   ...
   metadata:
     name: virtualization
   spec:
     enabled: true # Включить виртуализацию.
     settings:
       dvcr:
         storage:
...
| `virtualization-audit-*`      | master            | Доступен в коммерческих редакциях DP.                                                                     |
...
DP мигрирует машину один раз, и если миграция не удалась, владельцу машины придётся перенести или перезагрузить её самостоятельно.
{{< /alert >}}
