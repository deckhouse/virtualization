...
Рекомендуется использовать ядра Linux с актуальными обновлениями безопасности, предоставляемыми разработчиками выбранного дистрибутива. Такие обновления не являются обязательным условием работы виртуализации с точки зрения функциональности, но снижают риски для защищённости кластера в целом.
...
| `dvcr-*`                      | system            | На узлах должно быть доступно хранилище. При отсутствии system-узлов компонент размещается на worker-узлах. |
