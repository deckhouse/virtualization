...
5. Включите модуль `virtualization`:
   ...
   {{< alert level="warning" >}}
   Перед включением модуля внимательно ознакомьтесь с его настройками, которы описаны в [Руководстве администратора](./admin_guide.html#параметры-модуля).
   {{< /alert >}}
...
- только мастер-узлы и воркер-узлы;
- мастер-узлы, системные узлы и воркер-узлы;
- другие комбинации (в зависимости от архитектуры).

В таблице указаны компоненты модуля и типы узлов для их размещения. Компоненты распределяются по приоритету, только если соответствующие узлы доступны в конфигурации кластера.
