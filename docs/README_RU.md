...
{{< alert level="warning" >}}
Если вы планируете использовать модуль виртуализации в production-среде, рекомендуется разворачивать его на физических серверах. Развертывание модуля на виртуальных машинах также возможно, но в этом случае необходимо включить nested виртуализацию.
{{< /alert >}}
...
Локально из консоли документацию по ресурсам можно посмотреть, используя стандартную функциональность командной утилиты: `d8 kubectl explain <resource name>`
...
5. Установить консольную утилиту d8:
...
Модуль виртуализации использует пять каналов обновлений предназначенные для использования в разных окружениях, к которым с точки зрения надежности применяются разные требования:
...
- Ядро модуля (CORE), основанное на проекте KubeVirt и использующее QEMU/KVM + libvirtd для запуска виртуальных машин.
...
Класс виртуальной машины преднащначен для:
