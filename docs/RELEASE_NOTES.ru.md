...
- [vm] Добавлен сборщик завершившихся и неуспешных подов виртуальных машин:
       - поды старше чем 24 часа будут удалены;
       - хранится не более 2 завершённых подов.
- [usb] При ланирование виртуальных машин по узлам теперь учитывается используется ли USB-устройство USB 2.0 (High-Speed) или USB 3.0 (SuperSpeed).

### Исправления

- [usb] Стабилизирована работа USB-устройств для виртуализации на Deckhouse версии `>=1.76` и Kubernetes версии `>=1.33`.
- [usb] Исправлено обнаружение USB-устройств на хосте: ранее могли появляться дубликаты USB-устройств.
- [vm] Исправлено выселение виртуальных машин при дрейне узла: поды, отвечающие за подключение блок девайсов больше не пропадают с закордоненого узла до окончания миграции виртуальной машины.
- [vm] Блок девайсы можно подключать и отключать независимо от того, что виртуальная машина находится на закордоненом узле.
...
- [vm] В сттус виртуальной машины добавили отображение сообщений об обшибках, произошедших на стороне хранилища (CSI-драйвера) при подключении блочных устройств.
- [vd,vi,cvi] Исправили создание блочных устройств из VMDK (в особенности для VMDK в формате streamOptimized, используемых в экспортах из VMware).
