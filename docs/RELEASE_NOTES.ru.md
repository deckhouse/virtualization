...
- [vd] Исправлены ошибки при откате миграции [VirtualDisk](/modules/virtualization/cr.html#virtualdisk), если с диском было связано несколько PVC. Теперь контроллер проверяет имена исходного и целевого PVC перед выбором целевого.
- [vm] Исправлена живая миграция и вытеснение ВМ с дисками на локальном хранилище, подключёнными через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment) (hotplug). Целевой под миграции больше не размещается на исходном узле.
- [vm] Исправлено восстановление ВМ после неудачной миграции диска на локальном хранилище и рестарта. ВМ снова может запуститься без `ManualRecoveryRequired`. Автоматическое обновление размещения на узле откладывается до завершения миграции томов.
- [vm] Исправлена живая миграция ВМ с container-дисками при обновлении до v1.9. Теперь миграция проходит при переходе на rootless-режим: работает со старыми подами `virt-launcher` и монтирует container-диски на целевом узле.
- [vm] Исправлено ложное требование перезагрузки у ВМ только с сетью `Main` после обновления до v1.9. Теперь такие ВМ не получают статус `RestartRequired`, если конфигурация на самом деле не менялась.
