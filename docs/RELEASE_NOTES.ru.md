...
- [vm] Добавлена возможность таргетированной миграции для виртуальных машин. 
       Для этого нужно создать ресурс VirtualMachineOperation с типом Migrate и в нем указать .spec.migrate.nodeSelector для миграции машины на соответствующий узел.
...
- [clone] Испарвили невозможность успешно склонировать виртуальной машину, диски которой используют хранилище в режиме WaitForFirstConsumer.
- [module] Исправлен учёт ресурсов системных компонентов в квотах проекта (для работы виртуальных машин и их миграции).
- [module] Запретили удаление системных компонентов в несистемных неймспесах для пользовательских сервис аккаунтов.
- [vm] Исправили возможное зависание вирутальной машины в `Pending` в процессе миграции при смене класса хранения.
- [vd] Исправили смену класса хранения для виртуальных дисков, если происходит смена `Filesystem` на `Filesystem`.

### Прочее

- [vd] Добавили имя присоединенной виртуальной машины в расширенный вывод kubectl для виртуальных дисков (`kubectl get vd -owide`).
...
- [vd] Добавлена поддержка изменения класса хранения для дисков, подключённых через `VirtualMachineBlockDeviceAttachment` (hotplug).
...
- [module] Исправлен учёт ресурсов системных компонентов в квотах проекта (для создания дисков/образов и их подключенния к виртуальной машине через `VirtualMachineBlockDeviceAttachment` (hotplug)).
...
- [vm] Добавлена поддержку клонирования виртуальных машин в состоянии `Running` через `VirtualMachineOperation` с типом `Clone`.
...
- [vd] Исправлена проблема с восстановлением лейблов и аннтоаций на диске, созданном из снимка.
