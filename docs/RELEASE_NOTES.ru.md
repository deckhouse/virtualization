...
- [vd] Добавили возможность изменения класса хранения для дисков, подключенных через `VirtualMachineBlockDeviceAttachment` (hotplug).
- [vd] Добавили возможность миграции виртуальных машин с локальными дисками, подключенными через `VirtualMachineBlockDeviceAttachment` (hotplug).
- [vm] Виртуальную машину теперь можно запустить без `Main` сети.

### Исправления

- [module] В квотах проекта не учитываются ресурсы для системных компонентов, необходимых для создания дисков, образов и работы виртуальной машины
- [vi,cvi] Добавили отслеживание наличия образа в DVCR.
           Если образ пропадёт из DVCR, в статусе соответствующих ресурсов `VirtualImage` и `ClusterVirtualImage` будет соответствующая фаза `Lost` и ошибка.
- [vm] Подключение образов и дисков к виртуальной машине через `VirtualMachineBlockDeviceAttachment` (hotplug) больше не требует выделения дополнительного IP-адреса.
- [vmip] Исправлено присоединение IP-адреса, если соответствующий ресурс `VirtualMachineIPAddress` был создан пользователем самостоятельно и заранее.
- [vm] Добавили возможность клонирования запущенной виртуальной машины (в фазе `Running`) через `VirtualMachineOperation` с типом `Clone`.
