...
- [vm] Добавлен прогресс в процентах для операций по миграции виртуальной машины в статус ресурсов [VirtualMachineOperation](/modules/virtualization/cr.html#virtualmachineoperation) с типам `Evict` и `Migrate`.
  - Соответствующая колонка `PROGRESS` выводится при вызове `d8 k get vmop`.
- [vm] Добавлена колонка `PHASEAGE` при вызове `d8 k get vm`, отображающая возраст текущей фазы виртуальной машины. Старая колонка `AGE` сохранена.
- [vm] Добавлена возможность изменять CPU виртуальной машины без её ручной остановки.
  - Чтобы включить эту функциональность, необходимо добавить в `ModuleConfig` виртуализации соответствующий `HotplugCPUWithLiveMigration` в `.spec.settings.featureGates`.
  - Для изменения CPU без перезагрузки виртуальной машины будет применена живая миграция.
- [vm] Добавлена начальная поддержка изменения памяти виртуальной машины: изменение `.spec.memory` применяет новый размер памяти с помощью живой миграции.
  - Чтобы включить эту функциональность, необходимо добавить в `ModuleConfig` виртуализации соответствующий `HotplugMemoryWithLiveMigration` в `.spec.settings.featureGates`.
  - Для изменения памяти без перезагрузки виртуальной машины будет применена живая миграция.

### Исправления

- [vm] Миграция виртуальных машин переведена на `hostNetwork`, что заметно ускоряет процесс миграции.
- [vm] Исправлена проблема с неразмороженной файловой системой при снимке виртуальной машины, если заморозка произошла во время миграции.
- [vm] Исправлено удаление `Main` сети из виртуальной машины, чтобы виртуальная машина больше не использовала ip адрес из CIDR виртуализации.
- [api] При загрузке дисков и образов с типом `Upload` фаза `WaitForUserUpload` больше не призывает начать загрузку раньше времени, когда ресурс еще не готов для загрузки.
- [nsub] Добавлена автоматическая очистка отсутствующих на узле ресурсов [NodeUSBDevice](/modules/virtualization/cr.html#nodeusbdevice).
- [vmsnapshot] Исправлено создание снимка виртуальной машины без сети `Main`.

### Прочее

- [core] Исправлена уязвимость CVE-2026-39883.
- [core] Исправлены уязвимости CVE-2026-32280, CVE-2026-32281, CVE-2026-32282, CVE-2026-32283, CVE-2026-32288, CVE-2026-32289.
- [core] Исправлена уязвимость CVE-2026-34986.
- [core] Исправлены уязвимости CVE-2026-25679, CVE-2026-27142, CVE-2026-27139, CVE-2026-33186, CVE-2026-34040, CVE-2026-33997.
