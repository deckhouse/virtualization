...
- [core] Исправлена проблема в containerdv2, из-за которой хранилище, предоставляющее PVC с типом `FileSystem`, некорректно подключалось через `VirtualMachineBlockDeviceAttachment`.
...
- [vm] Добавлена валидация, которая запрещает указывать одну и ту же сеть в спецификации виртуальной машины `.spec.network` более одного раза.
