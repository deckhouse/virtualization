...
- [vmop] Для ресурсов [VirtualMachineOperation](/modules/virtualization/cr.html#virtualmachineoperation), прерванных более новой операцией над той же ВМ, добавлена фаза `Superseded`.
- [observability] Добавлены события безопасности для аудита виртуализации: управление модулем, жизненный цикл и доступ к ВМ, операции управления, проверки целостности и запрещённые операции.
...
- [module] Удалён CDI. Диски и образы теперь подготавливаются собственным импортером модуля и заполнителем PVC, с отображением прогресса импорта в статусе ресурса.
- [vd] Имена [VirtualDisk](/modules/virtualization/cr.html#virtualdisk) могут использовать полную длину имени Kubernetes вместо ограничения в 60 символов.
...
- [vmpool] Добавлен ресурс [VirtualMachinePool](/modules/virtualization/cr.html#virtualmachinepool) (EE/SE+) для декларативного управления группами одинаковых виртуальных машин. Пул масштабируется через стандартный сабресурс `scale`, HPA и KEDA.

### Исправления

- [vm] Исправлены сбои обновления состояния ВМ во время подключения образа через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment) (hotplug), из-за которых обновления ВМ могли прерываться в окне подключения.
- [vm] Исправлено периодическое переключение условия `VirtualMachineIPAddressReady`, когда у ВМ не было IP от агента гостевой ОС.
- [vmclass] Исправлено устаревание обнаруженных функций CPU для `cpu.type=Discovery`: функции пересчитываются по текущим узлам при каждом обновлении состояния, а `discovery.nodeSelector` больше не ограничивает планирование ВМ. Также восстановлено заполнение `status.cpuFeatures.notEnabledCommon` для типов `Discovery` и `Features`.
- [vd] Исправлено зависание [VirtualDisk](/modules/virtualization/cr.html#virtualdisk) с типом Upload в фазе `Pending` при недействительном сертификате общего хоста загрузки. Также снова работает автоматическое восстановление при смене хоста загрузки (например, после обновления `publicDomainTemplate`).
- [vi] Исправлено зависание [VirtualImage](/modules/virtualization/cr.html#virtualimage) и [ClusterVirtualImage](/modules/virtualization/cr.html#clustervirtualimage) с типом Upload в фазе `Pending` при недействительном сертификате общего хоста загрузки.
- [vi] Исправлен эндпоинт загрузки, который сохранял хост с момента создания ресурса вместо следования за изменениями `publicDomainTemplate`.
- [module] Ограничен неавторизованный доступ к порту USB/IP-шлюза виртуализации.
- [vm] Исправлено сохранение дисков и CD-ROM на шине SATA после включения паравиртуализации, из-за которого нельзя было отключить ISO с работающей ВМ. Устройства переводятся на шину `virtio-scsi` при перезапуске, который уже требуется для этого изменения.
- [vm] Исправлено завершение живых миграций по таймауту целевого узла во время ожидания свободного слота входящей миграции.
- [vm] Исправлено зависание ВМ до ручного удаления дочернего KVVMI в фазе `Failed`.
- [vm] Исправлена неинформативная ошибка при создании ВМ без явного класса, если не настроен [VirtualMachineClass](/modules/virtualization/cr.html#virtualmachineclass) по умолчанию. Сообщение об ошибке теперь явно указывает на отсутствие класса по умолчанию.
- [vm] Исправлена невозможность запуска ВМ после неудачного перезапуска миграции локального диска, когда `kvvmi` уже удалён, а мигрированные тома, в том числе подключённые через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment), нужно вернуть с целевых PVC на текущие исходные PVC.
...
- [vm] Исправлена неполная очистка томов, подключённых на лету (hotplug), после завершения целевого пода миграции, из-за которой могли оставаться устаревшие тома.
- [vm] Исправлена неполная диагностика ошибок томов пода: обработка теперь включает `FailedMapVolume` и показывает более полные сведения о сбое тома.
- [vm] Исправлена ошибка обновления CPU и памяти на лету (hotplug), когда квоты проекта не хватает для ресурсов на время миграции. Такие изменения теперь выполняются через перезапуск.
- [vmclass] Исправлена валидация политики размеров [VirtualMachineClass](/modules/virtualization/cr.html#virtualmachineclass): ошибки теперь сообщают итоговый объём памяти и поле, которое нужно изменить, а не значения на ядро; также применяются шаг числа ядер CPU и ограничения памяти только по минимуму.
