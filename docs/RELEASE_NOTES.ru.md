...
- [vmop] Операция над виртуальной машиной (ВМ), созданная без лейблов и аннотаций, больше не завершается ошибкой.
- [vm] ВМ с подключённым образом теперь можно мигрировать.
- [vm] Миграция ВМ со сбойным блочным устройством теперь называет причину и советует перезапуск.
- [vm] Автоматическое обновление ВМ больше не прекращается после первой неудачной миграции и повторяет её до трёх раз.
- [core] Служебные компоненты ВМ больше не остаются на исходном узле после живой миграции.
- [vd] Исправлена возможная потеря данных при миграции диска.
- [vd] Диск можно мигрировать повторно сразу после отменённой миграции.
- [vi] [ClusterVirtualImage](/modules/virtualization/cr.html#clustervirtualimage) больше не возвращается в фазу `Pending` во время загрузки.
- [core] Диск нулевого размера больше не блокирует операции с дисками и миграции ВМ.
- [dvcr] Исправлен завышенный размер, который модуль показывал для образов в формате `raw`.
- [module] Исправлена ошибка, из-за которой модуль не включался с первого раза.
...
- [vm] ВМ с дисками на SDS-хранилище теперь размещается на узле, где для них хватает места.
- [vd] Исправлено зависание ВМ с отключённой паравиртуализацией при запуске на классе хранения `WaitForFirstConsumer`.
