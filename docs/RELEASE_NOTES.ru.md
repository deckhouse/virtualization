...
- [clone] Исправлена проблема, из-за которой не удавалось успешно склонировать виртуальную машину, диски которой используют хранилище в режиме `WaitForFirstConsumer`.
- [module] Исправлен учёт ресурсов системных компонентов в квотах проекта (для работы виртуальных машин и их миграции).
- [module] Запрещено удаление системных компонентов в несистемных неймспейсах для пользовательских сервисных аккаунтов.
- [vm] Исправлено возможное зависание виртуальной машины в `Pending` в процессе миграции при смене класса хранения.
- [vd] Исправлена смена класса хранения для виртуальных дисков, если происходит смена `Filesystem` на `Filesystem`.

### Прочее

- [vd] Добавлено имя присоединённой виртуальной машины в расширенный вывод `kubectl` для виртуальных дисков (`kubectl get vd -owide`).
...
- [vd] Добавлена поддержка изменения класса хранения для дисков, подключённых через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment) (hotplug).
