...
- [vmsop] Доабвлен ресурс `VirtualMachineSnapshotOperations` для создания виртуальной машины не основе снимка `VirtualMachineSnapshot`.
...
- [module] Для ModuleConfig вирутализации добавлена валидация, запрещающая уменьшение размера и изменение выбранного класса хранения для dvcr.
- [module] Улучшили события аудита, используя более информативные сообщения, включающие имена виртуальных машин и информацию о пользователях.
- [module] Добавлена возможность сборки мусора для dvcr по расписанию:
  - по умолчанию эта функция отключена;
  - чтобы включить очистку, необходимо задать расписание в настройках модуля: .spec.settings.dvcr.gc.schedule.
...
- [???] [observability] Добавлены новые метрики для дисков:
...
- [vmclass] Исправили возможность изменения или удаления `VirtualMachineClass` generic. Теперь модуль виртуализации не будет его восстанавливать в исходное состояние.
- [vm] Исправили ошибку MethodNotAllowed для операций patch и watch для запросов к ресурсу "virtualmachine" через kubectl.
- [image] Исправили невозможность удаления ресурсов `VirtualImage` и `ClusterVirtualImage` для остановленной виртуальной машины.
- [module] Исправили rbac для кластерных ролей user и editor.
