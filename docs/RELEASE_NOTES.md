...
- [observability] Added information about pods associated with virtual machines to the virtual machine dashboard.
...
- [vd] Fixed virtual disk creation hanging in `WaitForFirstConsumer` mode on nodes with taints.
...
- [vm] Added the `--from-file` flag to the `vlctl` utility to view domain information directly from a local libvirt XML file.
