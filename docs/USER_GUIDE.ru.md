...
| `.spec.cpu.cores`                       | Обычно через живую миграцию, иначе требуется перезапуск ВМ |
...
Горячее подключение CPU позволяет изменять `spec.cpu.cores` у работающей ВМ без перезагрузки, если изменение можно применить через живую миграцию. В пределах текущей топологии CPU можно как увеличивать, так и уменьшать количество ядер.

Чтобы включить эту функциональность, добавьте `HotplugCPUWithLiveMigration` в `.spec.settings.featureGates` объекта ModuleConfig модуля `virtualization`:
...
Если новое значение `spec.cpu.cores` попадает в диапазон горячего подключения для текущей топологии и ВМ поддерживает миграцию, изменение применяется через живую миграцию. Если новое значение требует изменения топологии CPU или ВМ не поддерживает миграцию, потребуется перезагрузка ВМ. Необходимость перезагрузки отображается в условии `AwaitingRestartToApplyConfiguration`.

Особенности гостевых ОС:

- После живой миграции новые vCPU могут потребовать явной активации в гостевой ОС.
- Наличие в гостевой ОС настроек автоматического включения новых ядер не влияет на их активацию при перезагрузке.
...
- Чтобы автоматически включать новые CPU в Linux, можно настроить `udev`-правило. После этого добавленные ядра будут видны в `cat /proc/cpuinfo` и `top`:
