...
- `Pending` - ВМ только что создана, перезапущена или запущена после остановки и ожидает готовности необходимых ресурсов (дисков, образов, ip-адресов и т.д.).
  - Возможные проблемы:
    - не готовы зависимые ресурсы: диски, образы, классы ВМ, секрет со сценарием начальной конфигурации и пр.
  - Диагностика: В `.status.conditions` можно увидеть, что блокирует переход к следующей фазе, например, ожидание готовности дисков (BlockDevicesReady) или класса ВМ (VirtualMachineClassReady).
- `Starting` - все зависимые ресурсы ВМ - готовы, и система пытается запустить ВМ на одном из узлов кластера.
  - Возможные проблемы:
...
- `Running` - ВМ успешно запущена и работает.
  - Особенности:
    ...
    - Условие `type: FirmwareUpToDate` отображает информацию о текущем состоянии прошивки ВМ.
    - Условие `type: AwaitingRestartToApplyConfiguration` отображает информацию о необходимости выполнить вручную перезагрузку ВМ, т.к. есть изменения в конфигурации, которые невозможно применить без перезагрузки.
    - Условие `type: ConfigurationApplied` отображает информацию, о том что конфигурация применена для запущенной ВМ.
  - Возможные проблемы:
...
- `Migrating` - ВМ переносится на другой узел кластера (живая миграция).
  - Особенности:
...
Будьте внимательны и учитывайте данные особенности, при настройке параметров sizingPolicy для [VirtualMachineClass](cr.html#virtualmachineclass).
...
Миграция ВМ работает следующим образом:

1. **Создание нового экземпляра ВМ**

    На целевом узле запускается новая ВМ в приостановленном состоянии. Её конфигурация (CPU, диски, сеть) копируется с исходного хоста.

2. **Первичная передача памяти**

    Вся оперативная память исходной ВМ копируется на целевой узел через сеть.

3. **Отслеживание изменений (Dirty Pages)**

    В процессе передачи памяти ВМ продолжает работать, модифицируя часть страниц. Гипервизор помечает эти страницы как «грязные» (dirty pages).

4. **Итеративная синхронизация**

    После завершения первичной передачи запускаются циклы повторной передачи только измененных (dirty) страниц. Количество итераций зависит от активности ВМ:
    - Чем выше нагрузка, тем больше изменений и дольше миграция.
    - Каждый цикл при достаточной пропускной способности сокращает объем несинхронизированных данных.

5. **Финальная синхронизация и переключение**

    Когда объем dirty pages становится достаточно мал, исходная ВМ приостанавливается (обычно на 100 мс):
    - Оставшиеся изменения памяти передаются на целевой узел.
    - Состояние CPU, устройств и открытых соединений синхронизируется.
    - ВМ запускается на целевом узле, а исходная копия удаляется.

Пропускная способность сети напрямую влияет на время миграции: низкая скорость увеличивает количество итераций и простой ВМ, также при недостаточной пропускной способности сети миграция может не завершиться.

Чтобы гарантировать завершение миграции даже при низкой пропускной способности сети, можно использовать механизм AutoConverge. Этот механизм:

1. **Тормозит CPU ВМ**
  - Постепенно снижает частоту CPU исходной ВМ, замедляя генерацию dirty pages.
  - Уровень торможения адаптируется: чем выше нагрузка, тем сильнее замедление.
2.** Ускоряет синхронизацию**
  - Позволяет миграции «догнать» изменения, сокращая объем неотправленных данных.
3. **Автоматически завершает миграцию**
- После активации AutoConverge финальная синхронизация запускается, когда скорость передачи превышает скорость изменения памяти.

Параметр `.spec.liveMigrationPolicy` виртуальной машины позволяет конфигурировать поведение при миграции ВМ.

- `AlwaysSafe` - всегда выполнять живую миграцию виртуальной машины без торможения CPU.
- `PreferSafe` - по умолчанию выполнять живую миграцию виртуальной машины без торможения CPU, но можно переопределять через VirtualMachineOperation (type=Evict, force = true)
- `AlwaysForced` - всегда выполнять живую миграцию виртуальной машины c торможением CPU.
- `PreferForced` - по умолчанию выполнять живую миграцию c торможением CPU, но можно переопределить в VirtualMachineOperation (type=Evict,force = false)
