...
- Устройства из `.spec.blockDeviceRefs` используют шину SATA; у работающей ВМ изменения списка (добавление или удаление диска, в том числе ISO-образа) вступают в силу после перезагрузки ВМ. Учитывайте это при работе через веб-интерфейс и при автоматизации.
...
- параметры сети (IP-адреса, шлюзы, DNS и т.д.) для дополнительных сетей настраиваются вручную изнутри гостевой ОС (например, с помощью Cloud-Init), если для сети не настроен IPAM (см. [IPAM для дополнительных сетевых интерфейсов](#ipam-для-дополнительных-сетевых-интерфейсов)).
...
Если в модуле `sdn` для дополнительной сети настроен IPAM (пул IP-адресов, привязанный к сети через `spec.ipam.ipAddressPoolRef`), модуль виртуализации может автоматически выделять IP-адреса для дополнительных интерфейсов ВМ и доставлять их в гостевую ОС через DHCP.

Поддерживаются два режима:

- **Автоматический (DHCP):** если `ipAddressName` не указан в `.spec.networks[]`, контроллер автоматически создаёт ресурс `IPAddress` (тип `Auto`), привязанный к ВМ через `ownerReferences`, и передаёт его в модуль SDN. SDN выделяет адрес из пула и доставляет его в гостевую ОС через DHCP. Адрес стабилен при перезагрузках и миграции ВМ (привязан к ВМ, а не к pod'у). В гостевой ОС на соответствующем интерфейсе должен быть включён DHCP-клиент.

- **Статический:** если `ipAddressName` указан в `.spec.networks[]`, контроллер использует предоставленный пользователем ресурс `IPAddress` (тип `Static`, `network.deckhouse.io/v1alpha1`). Адрес определяется пользователем и стабилен.

Если у дополнительной сети не настроен пул IPAM, функция IPAM не включается — интерфейс работает в режиме L2-only, а IP-адресация настраивается вручную в гостевой ОС.

{{< alert level="info" >}}
Поле `ipAddressName` в `.spec.networks[]` определяет режим: если пусто — автоматический режим (контроллер управляет `IPAddress`); если задано — статический режим (пользователь управляет `IPAddress`).
{{< /alert >}}

Пример ВМ с автоматическим выделением IP-адреса на дополнительной сети:
...
{{< alert level="warning" >}}
Если IPAM включён (у сети есть пул), не настраивайте статический IP-адрес на дополнительном интерфейсе в гостевой ОС вручную (через Cloud-Init). Используйте автоматический (DHCP) или статический (`ipAddressName`) режим, чтобы избежать конфликтов адресов.
{{< /alert >}}

{{< alert level="info" >}}
Если у дополнительной сети есть пул IPAM, но ресурс `IPAddress` ещё не выделен или находится в состоянии `Pending` (например, пул исчерпан), интерфейс временно пропускается — ВМ запускается без него, а в condition `NetworkReady` сообщается об ошибке. Когда адрес становится доступным, интерфейс подключается автоматически (hotplug).
{{< /alert >}}
