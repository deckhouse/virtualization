...
Условие `type: Migratable` в статусе ВМ отвечает на один вопрос: получится ли перенести эту машину живой миграцией прямо сейчас. Оно учитывает и саму машину (диски, проброшенные устройства, тип процессора), и состояние кластера — есть ли узел, на который её можно перенести.

Общая картина по всем машинам:

```bash
d8 k get vm -o wide
```

Значение в колонке `MIGRATABLE` показывает результат, а причина — в условии:
...
| --- | --- | --- |
| `VirtualMachineMigratable` | Машину можно перенести живой миграцией | — |
| `VirtualMachineNoMigrationTarget` | Сама машина к миграции пригодна, но переносить её некуда: ни один другой узел кластера не подходит под её правила размещения | Проверьте `spec.nodeSelector`, `spec.affinity` и `spec.tolerations` машины и такие же параметры её `VirtualMachineClass`. Также узел может быть недоступен: выведен из планирования или на нём не работает виртуализация |
| `VirtualMachineDisksNotMigratable` | Диски машины лежат в хранилище, доступном только с одного узла | Перенесите диски в хранилище с режимом доступа `ReadWriteMany` |
| `VirtualMachineHostDevicesNotMigratable` | К машине подключено устройство, которое нельзя перенести на другой узел | Отключите устройство и перезапустите машину |
| `VirtualMachineDisksShouldBeMigrating` | Машину можно перенести: её локальные диски поедут вместе с ней | — |

#### Проверка перед выводом узла на обслуживание

Машины, которые не смогут уехать с узла, лучше найти заранее — до того, как узел выведен из планирования и обслуживание уже началось:

```bash
d8 k get vm -o wide | grep <node-name>
```

Машины со значением `False` в колонке `MIGRATABLE` при выводе узла придётся остановить: живая миграция для них невозможна, и эвакуация завершится ошибкой.

#### Что нужно знать об условии `Migratable`

- **Условие может измениться само по себе, без изменений в ВМ.** Оно описывает не только машину, но и кластер: если единственный подходящий узел выведен из планирования или с него снята нужная метка, условие станет `False`, хотя саму машину никто не трогал. Когда подходящий узел появится снова, условие вернётся в `True`.

- **У остановленной ВМ условия нет.** Пригодность к миграции вычисляется для запущенной машины, поэтому у остановленной условие отсутствует, а не показывает результат прошлого запуска. Это важно: за время простоя диски могли переехать в другое хранилище, а устройство — быть отключено, и старый ответ был бы неверным.

- **Изменения параметров размещения учитываются после перезапуска ВМ.** Пока машина работает, она использует те параметры, с которыми была запущена, — условие описывает именно их. Если вы поменяли `nodeSelector`, `affinity` или класс ВМ, условие пересчитается по новым правилам после перезапуска машины.

- **Локальные диски не мешают миграции, но узел всё равно нужен.** Машина с локальными дисками переносится вместе с ними, поэтому условие остаётся `True` с причиной `VirtualMachineDisksShouldBeMigrating`. Но если подходящего узла в кластере нет, условие будет `False` — переносить диски вместе с машиной некуда.
