...
Чтобы указать целевой узел, на котором необходимо разместить виртуальную машину, необходимо указать соответствуюище метки этого узла в опции `--node-selector` в формате `key=value`. Метки могут быть перечислены через запятую, если их несколько.

Запустить миграцию можно также создав ресурс VirtualMachineOperations (`vmop`) с типом `Migrate` вручную:
...
Примечание: Если нет необходимости указывать параметры целевого узла, то можно выселить виртуальную машину на другой подходящий узел с помощью команды `d8 v evict` или создав ресуср `vmop` с типом `Evict`.
...
Пример выполнения миграции той же виртуальной машины с использованием флага `--force` команды `d8 v migrate` (который включает механизм AutoConverge): здесь хорошо видно, что частота процессора снижается поэтапно, чтобы уменьшить скорость изменения памяти.
