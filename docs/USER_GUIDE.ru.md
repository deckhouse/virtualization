...
После создания, ресурс перейдет в фазу `WaitForUserUpload`, а это значит, что он готов для загрузки образа. Если загрузка не начнётся в течение 30 минут, ресурс перейдет в фазу `Failed`; для повторной попытки пересоздайте ресурс.
...
После создания ресурс перейдет в фазу `WaitForUserUpload`, что означает, что он готов для загрузки диска. Если загрузка не начнётся в течение 30 минут, ресурс перейдет в фазу `Failed`; для повторной попытки пересоздайте ресурс.
