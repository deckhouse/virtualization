...
{{< alert level="warning">}}
Обратите внимание, что `VirtualImage` — это проектный ресурс, то есть он доступен только в том проекте или пространстве имен, в котором был создан. Для использования образов на уровне всего кластера предназначен отдельный ресурс — [`ClusterVirtualImage`](./admin_guide.html#образы).
{{< /alert >}}
...
- STOREDSIZE (размер в хранилище) — размер образа, который фактически занимает место в хранилище (DVCR или PVC). Для сжатых образов (gz, xz) этот размер меньше, чем распакованный размер.
- UNPACKEDSIZE (распакованный размер) — размер образа после распаковки, который будет использоваться при создании диска из этого образа. Этот размер определяет минимальный размер диска, который можно создать из образа.

{{< alert level="info" >}}
При создании диска из образа необходимо указать размер диска, который должен быть равен или больше значения UNPACKEDSIZE. Если размер не указан, диск будет создан с размером, соответствующим распакованному размеру образа.
{{< /alert >}}
...
- `Exporting` - идет процесс экспорта диска.
...
{{< alert level="warning" >}}
TPM-устройство, предоставляемое виртуальной машине, не является постоянным (эмуляция TPM в памяти). Это означает, что при перезагрузке или миграции ВМ состояние TPM сбрасывается.

Рекомендуется учитывать это ограничение при планировании использования функций безопасности Windows, зависящих от TPM.
{{< /alert >}}
...
{{< alert level="info" >}}
Для большинства современных Linux-дистрибутивов рекомендуется использовать `bootloader: EFI`. Для Windows обычно требуется `bootloader: EFI` или `bootloader: EFIWithSecureBoot`.
{{< /alert >}}
...
#### Tolerations (толерантность к ограничениям узлов)

`Tolerations` позволяют ВМ запускаться на узлах с ограничениями (`taints`), которые в противном случае блокировали бы размещение ВМ. Это полезно, когда нужно разрешить запуск ВМ на специальных узлах (например, узлах для тестирования или узлах с определенными характеристиками).
...
{{< alert level="warning" >}}
Для просмотра информации об узлах кластера (включая `taints`) требуется соответствующая роль пользователя с правами доступа к ресурсам уровня кластера.
{{< /alert >}}
...
#### PriorityClassName (приоритет планирования)

`PriorityClassName` определяет приоритет ВМ при планировании на узлы. ВМ с более высоким приоритетом могут вытеснять ВМ с более низким приоритетом при нехватке ресурсов на узле.
...
Чтобы посмотреть доступные классы приоритета в кластере, выполните команду:
...
{{< alert level="info" >}}
Если указанный класс приоритета не существует, ВМ не сможет быть запущена.
{{< /alert >}}
...
Для размещения ВМ по зонам доступности вместо узлов используйте `topologyKey: "topology.kubernetes.io/zone"` (см. [раздел «Размещение ВМ по зонам доступности»](#размещение-вм-по-зонам-доступности)).
...
{{< alert level="warning">}}
Имена блочных устройств (`/dev/sda`, `/dev/sdb`, `/dev/sdc` и т.д.) присваиваются ядром Linux в порядке обнаружения устройств при загрузке. Порядок обнаружения может меняться между перезагрузками, что приводит к изменению имён дисков даже при неизменных SCSI-адресах.

Использование имён `/dev/sdX` в конфигурационных файлах (например, `/etc/fstab`) или в скриптах может привести к монтированию неправильных дисков или некорректной работе после перезагрузки ВМ.
{{< /alert >}}
...
- **SCSI-адреса** (`0:0:0:X`) — для программной идентификации дисков

В конфигурационных файлах и скриптах используйте UUID разделов или симлинки из `/dev/disk/by-*` вместо имён `/dev/sdX`.
