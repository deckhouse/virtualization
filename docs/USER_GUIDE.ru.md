...
- `false` — используется эмуляция стандартных устройств (SATA для дисков, e1000e для сетевых интерфейсов), что может быть необходимо для совместимости со старыми ОС без драйверов `VirtIO`. Добавление устройства в `.spec.blockDeviceRefs` у запущенной ВМ, как правило, выполняется без перезагрузки. Удаление устройства из списка (в том числе извлечение ISO-образа) потребует перезагрузки ВМ, так как горячее отключение дисков в этом сценарии опирается на шину `virtio-scsi`, а не на SATA. Горячее подключение через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment) (`vmbda`) по-прежнему доступно.
...
При `enableParavirtualization: true` оба способа позволяют подключать и отключать диски у работающей ВМ без перезагрузки, если диск доступен на узле, где она выполняется.

{{< alert level="warning" >}}
Если у ВМ указано `enableParavirtualization: false`, работа с дисками отличается от режима по умолчанию:

- для устройств из `.spec.blockDeviceRefs` используется SATA: диск или образ к запущенной ВМ, как правило, можно добавить без перезагрузки, а удаление из списка (отключение диска, извлечение ISO-образа) потребует перезагрузки ВМ. Учитывайте это при работе через веб-интерфейс или при автоматизации;
- для дисков, подключённых через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment), по-прежнему используется шина `virtio-scsi`, поэтому горячее подключение и отключение сохраняются, но в гостевой ОС должны быть установлены драйверы `VirtIO` для SCSI-контроллера — иначе диск может не отображаться в системе, хотя ресурс `vmbda` в кластере создан корректно.

Полная поддержка hotplug доступна при `enableParavirtualization: true`.
{{< /alert >}}
...
Изменение порядка устройств в списке или значений `bootOrder` вступает в силу после перезагрузки ВМ. Например, можно подключить ISO-образ для установки ОС с нужным приоритетом загрузки, а после установки удалить его из списка. Если у ВМ `enableParavirtualization: false`, удаление ISO из `.spec.blockDeviceRefs` может потребовать перезагрузки ВМ.
...
Для отключения диска удалите его из списка. Если у ВМ задано `enableParavirtualization: false`, отключение через `.spec.blockDeviceRefs` может потребовать перезагрузки ВМ.
