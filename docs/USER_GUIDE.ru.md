...
Пример ресурса Secret со сценарием Sysprep приведён ниже:
...
- Учитывайте существующие ограничения (`taints`) на узлах. При необходимости можно добавить соответствующие `tolerations` для ВМ. Пример использования `tolerations` для разрешения запуска на узлах с taint `node.deckhouse.io/group=:NoSchedule` приведён ниже:
...
- очистить `machine-id`: `sudo truncate -s 0 /etc/machine-id` (для systemd) или удалить файл `/var/lib/dbus/machine-id`.
