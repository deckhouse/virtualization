...
- `false` — используется эмуляция стандартных устройств (SATA для дисков, e1000e для сетевых интерфейсов), что может быть необходимо для совместимости со старыми ОС. Горячее подключение блочных устройств через `.spec.blockDeviceRefs` в этом режиме не поддерживается — изменения требуют перезагрузки ВМ. Горячее подключение через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment) по-прежнему поддерживается.
...
- через [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment) (`vmbda`) — диск подключается отдельным ресурсом и не участвует в порядке загрузки. Диски всегда подключаются через шину SCSI независимо от значения `enableParavirtualization`. Рекомендуется для автоматизации и когда нет прав на редактирование ВМ.

Оба способа поддерживают hotplug (добавление и удаление устройства без перезагрузки ВМ).

{{< alert level="warning" >}}
Для ВМ с `enableParavirtualization: false` горячее подключение через `.spec.blockDeviceRefs` не поддерживается. Изменения списка блочных устройств требуют перезагрузки ВМ. Для горячего подключения дисков к непаравиртуализированным ВМ используйте [VirtualMachineBlockDeviceAttachment](/modules/virtualization/cr.html#virtualmachineblockdeviceattachment).
{{< /alert >}}
