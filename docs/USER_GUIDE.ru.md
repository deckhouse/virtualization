...
- Планируйте резервные узлы для критичных нагрузок. Для ВМ с жесткими требованиями (например, AntiAffinity) предусмотрите дополнительные узлы, чтобы избежать простоев при сбое или выводе узла в режим обслуживания.
...
В этом примере в кластере 3 узла, два с быстрыми дисками (`disktype=ssd`) и один с медленными (`disktype=hdd`). Виртуальная машина будет размещена только на узлах, которые имеют метку `disktype` со значением `ssd`.
...
- Жёстким (`requiredDuringSchedulingIgnoredDuringExecution`) — ВМ размещается только на узлах, удовлетворяющих условию.
- Мягким (`preferredDuringSchedulingIgnoredDuringExecution`) — ВМ размещается на подходящих узлах, если это возможно.

`nodeAffinity` - определяет, на каких узлах может быть запущена ВМ, используя выражения меток.
...
Если использовать мягкое требовние (`preferredDuringSchedulingIgnoredDuringExecution`) для этого примера, то в случае если по какой-то причине ВМ не может быть запущена на узлах с быстрыми дисками, например нет ресурсов, то она будет запланирована на узле с медленными дисками.
...
Если неуказывать значение `nodePort` явно, для сервиса будет назначен произвольный порт, который можно посмотреть в статусе сервиса, сразу после его создания.
...
По умолчанию параметр `requiredConsistency` установлен в значение `true`.
Если целостность данных не критична, снимок можно создать на работающей ВМ без заморозки ФС (в этом случае наличие qemu-guest-agent не обязательно). Для этого укажите в спецификации:
