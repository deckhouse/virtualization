...
Создайте пул с нужным числом реплик и шаблоном. Диски на каждую реплику описываются в `virtualDiskTemplates`. Их порядок задаёт порядок устройств реплики (порядок загрузки): первый шаблон — загрузочный диск. У шаблона пула нет поля `blockDeviceRefs`: контроллер выдаёт каждой реплике собственную копию каждого шаблона и подключает их сам, поэтому диски описываются один раз.
...
- Каждое устройство создаётся на каждую реплику. Общего блочного устройства (единого образа/ISO на все реплики) нет: описывайте устройство как запись `virtualDiskTemplates`, каждая реплика получит свою копию.
