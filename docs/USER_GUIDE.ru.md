...
Чтобы разместить виртуальную машину на конкретном целевом узле, укажите имя этого узла в опции `--target-node-name`.
...
{{< alert level="info" >}}
Если нет необходимости указывать параметры целевого узла, виртуальную машину можно выселить на другой подходящий узел с помощью команды `d8 v evict` или создав ресурс [VirtualMachineOperation](/modules/virtualization/cr.html#virtualmachineoperation) с типом `Evict`.
{{< /alert >}}
