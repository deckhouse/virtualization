...
Горячее подключение диска невозможно, пока виртуальная машина находится в процессе живой миграции. В этом случае ресурс VirtualMachineBlockDeviceAttachment остаётся в фазе `Pending`, а в условии `Attached` указывается причина `BlockedByMigration`. Подключение продолжится после завершения миграции.
