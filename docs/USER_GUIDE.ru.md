...
{{< alert level="warning" >}}
Использование образа с параметром `storage: PersistentVolumeClaim` разрешено для создания дисков, соответствующих тому же классу хранения (StorageClass).
{{< /alert >}}
...
В зависимости от свойств хранилища, поведение дисков при создании виртуальных машин в процессе эксплуатации может отличаться:
...
- `Strict` - режим строго восстановления, когда требуется восстановление ВМ "как в снимке", отсутствующие внешние зависимости могут привести ктому, что ВМ после восстановления будет в Pending.
- `BestEffort` - игнорируются отсутствующие внешние зависимости (ClusterVirtualImage, VirtualImage) удаляются из конфигурации ВМ.

Восстановление виртуальной машины из снимка возможно при выполнении следующих условий:
- Восстанавливаемая ВМ присутствует в кластере (существует ресурс `VirtualMachine`, а его `.metadata.uid` совпадает с идентификатором, использованным при создании снимка).
- Восстанавливаемые диски (идентифицируются по имени) не подключены к другим ВМ и отсутствуют в кластере.
- Восстанавливаемый IP-адрес не занят другой ВМ и является свободным.
- Восстанавливаемые MAC-адреса не используются другими ВМ и являются свободными.

{{< alert level="warning" >}}
Если некоторые ресурсы, от которых зависит ВМ (например, VirtualMachineClass, VirtualImage, ClusterVirtualImage), отсутствуют в кластере, но существовали на момент создания снимка, ВМ после восстановления перейдет в состояние Pending.
В этом случае потребуется вручную отредактировать конфигурацию ВМ и изменить или удалить зависимые ресурсы, которые отсуствуют в кластере.
{{< /alert >}}
...
Платформа позволяет экспортировать диски и снимки дисков виртуальных машин с использованием утилиты `d8` (версия 1.17 и выше).

Пример команды для экспорта диска (выполняется на узле кластера):

```bash
d8 download -n <namespace> vd/<virtual-disk-name> -o file.img
```

Пример команды для экспорта снимка диска (выполняется на узле кластера):

```bash
d8 download -n <namespace> vds/<virtual-disksnapshot-name> -o file.img
```

Для экспорта ресурсов за пределами кластера необходимо дополнитьельно использовать ключ `--publish`
