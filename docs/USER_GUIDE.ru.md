...
{{< alert level="warning" >}}
`EFIWithSecureBoot` хранит состояние Secure Boot в постоянном томе, поэтому в кластере должен быть StorageClass по умолчанию. Если его нет, виртуальная машина остаётся в состоянии `Pending` (в условии `Running` отображается сообщение `no default storage class found`), пока StorageClass по умолчанию не будет задан.
{{< /alert >}}
