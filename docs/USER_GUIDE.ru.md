...
DVP позволяет экспортировать диски и снимки дисков виртуальных машин с использованием утилиты `d8` (версия 0.20.7 и выше).
Для этого должен быть включен модуль [storage-volume-data-manager](https://deckhouse.ru/modules/storage-volume-data-manager/stable/)
Диск не должен использоваться в момент экспорта. Если подключен к ВМ, то ВМ надо остановить.
