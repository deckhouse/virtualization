...
Для платных редацкий платформы существует возможность изменения класса хранения для соданных дисков. На данный момент это возможно только для работающих ВМ (фаза ВМ должна быть `Running`).
...
После изменения конфигурации диска запустится живая миграция ВМ, в процессе которой диск ВМ будет смигрирован на нове хранилище.

Если к виртуальной машине подключены несколько дисков и треубется изменить класс хранения для нескольких дисков, эту операцию необходимо выполнить последовательно:
...
Дополнительные сети задаются в конфигурационном блоке .spec.networks. Если этот блок отсутствует (значение по умолчанию), ВМ подключается исключительно к основной сети кластера.
...
- VirtualMachineMACAddress - MAC адрес дополнительныих сетевых интерфейсов, при подключении ВМ к дополнительным сетям.
