...
## Поддержка дополнительных сетевых интерфейсов

Наряду с основным сетевым интерфейсом, подключённым к default-сети (сервисной сети кластера), виртуальные машины (ВМ) могут быть подключены к дополнительным сетям — Network (на уровне проекта) или ClusterNetwork (на уровне кластера). 

### Основные принципы

Дополнительные сети подключаются к виртуальной машине (ВМ) через блок `.spec.networks`. Каждая сеть может быть подключена несколько раз, при этом подключение и отключение сетей возможно как для остановленной, так и для запущенной ВМ.

**Важно:**  
Изменения в конфигурации дополнительных сетей на запущенной ВМ вступают в силу только после перезагрузки.

**Поддерживаемые действия:**
- **Подключение** дополнительного сетевого интерфейса — выполняется путём добавления в список `.spec.networks` записи с указанием имени и типа сети.
- **Отключение** дополнительного сетевого интерфейса — выполняется путём удаления соответствующей записи из списка `.spec.networks`.

**Рекомендация:**  
Чтобы избежать изменения порядка сетевых интерфейсов внутри операционной системы, рекомендуется добавлять новые сети в конец списка `.spec.networks`.
...
Информация о подключённых сетях и их MAC-адресах отображается в статусе ВМ:
...
Для каждого дополнительного сетевого интерфейса автоматически резервируется **уникальный MAC-адрес** с использованием специализированных ресурсов: `VirtualMachineMACAddress` (vmmac) и `VirtualMachineMACAddressLease` (vmmacl). Это обеспечивает стабильность и отсутствие коллизий в L2-сетях.
...
- Первые три октета (OUI) формируются на основе UUID кластера, последние три (NIC) — случайно из 16 миллионов возможных комбинаций.

Ресурс `VirtualMachineMACAddressLease` (`vmmacl`): кластерный ресурс, который управляет арендой MAC-адресов из общего пула.
...
Ресурс `VirtualMachineMACAddress` (`vmmac`): проектный/неймспейсный ресурс, который отвечает за резервирование арендованных MAC-адресов и их привязку к виртуальным машинам.
...
- Функциональность управления дополнительными сетями доступна **только при включённом модуле `d8-sdn`**.
  - Если модуль `d8-sdn` не активен, ВМ с дополнительными сетевыми интерфейсами остаётся в фазе `Pending`.
  - В условии `NetworkReady` будет указано сообщение об ошибке: *"Дополнительные сети недоступны: требуется включённый модуль d8-sdn"*.
- Порядок сетей в списке определяет порядок подключения к шине ВМ.
- Настройка IP-адресов в дополнительных сетях (шлюзы, DNS и т.п.) **выполняется вручную** внутри гостевой ОС (например, через `cloud-init`).
- При удалении сети из конфигурации:
  - MAC-адрес освобождается.
  - Соответствующие ресурсы `vmmac` и `vmmacl` удаляются.
- Для совместимости:
  - При обновлении существующих ВМ до версии, поддерживающей `vmmac` и `vmmacl`, контроллер автоматически создаёт соответствующие ресурсы.
  - Исходные MAC-адреса сохраняются и регистрируются в новых ресурсах, обеспечивая непрерывность.
