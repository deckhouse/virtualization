...
QEMU Guest Agent поддерживает скрипты, которые выполняются на гостевой ОС перед и после операций заморозки (freeze) и оттаивания (thaw) файловой системы. Это позволяет подготовить приложения к созданию снимка без остановки сервисов, обеспечивая согласованное состояние обеспечивающееся на уровне приложений.

Скрипты размещаются в каталоге hooks гостевой системы и должны иметь права на выполнение. Расположение каталога зависит от дистрибутива:

- `/etc/qemu-ga/hooks.d/` — в большинстве дистрибутивов (Debian, Ubuntu и др.);
- `/etc/qemu/fsfreeze-hook.d/` — в некоторых дистрибутивах (RHEL, CentOS, Fedora и др.).
