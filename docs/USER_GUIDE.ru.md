...
Данное руководство предназначено для пользователей модуля виртуализации Deckhouse и описывает порядок создания и изменения ресурсов, которые доступны для создания в проектах и пространствах имен кластера.
...
Как узнать доступные варианты хранилищ веб-интерфейсе:
...
QEMU Guest Agent не требует дополнительной настройки после установки. Однако для обеспечения консистентности снимков на уровне приложений (без остановки сервисов) можно настроить скрипты hooks, которые выполняются на гостевой ОС перед и после операций заморозки (freeze) и оттаивания (thaw) файловой системы. Скрипты размещаются в каталоге hooks гостевой системы и должны иметь права на выполнение. Расположение каталога зависит от дистрибутива:
...
Для живой миграции устанавливается таймаут завершения, который рассчитывается по формуле:

**Таймаут завершения = completionTimeoutPerGiB × (Размер памяти в GiB + Размер диска в GiB (если используется Block Migration))**

Если миграция не завершается в течение этого времени, операция считается неудачной и отменяется. Параметр `completionTimeoutPerGiB` настраивается в конфигурации модуля виртуализации.

**Пример расчета:**

Для виртуальной машины с 4 GiB памяти и 20 GiB диска, при значении `completionTimeoutPerGiB` равном 30 секундам, таймаут составит:

30 секунд × (4 GiB + 20 GiB) = 30 × 24 = 720 секунд (12 минут)
...
Флаг `--force` при выполнении миграции виртуальной машины активирует специальный механизм — [autoconverge](#механизм-autoconverge). Этот механизм автоматически снижает нагрузку на процессор виртуальной машины (замедляет её CPU), если требуется ускорить завершение миграции и обеспечить её успешное выполнение, даже в случае, когда пересылка памяти ВМ идёт слишком медленно. Используйте этот флаг, если стандартная миграция не может завершиться из-за высокой активности ВМ.
...
Чтобы понять, что пропускной способности сети не хватает для живой миграции ВМ, ориентируйтесь на графики "Namespace / Virtual Machine" -> "VM Status details" -> "Live migration memory metrics": если скорость передачи памяти (**Processed memory rate**) меньше скорости изменения памяти (**Dirty memory rate**), а значение оставшейся памяти (**Remaining memory rate**) долго не убывает, это значит, что сеть стала узким местом для миграции.

Пример ситуации, когда миграция не может быть завершена из-за недостаточной пропускной способности сети: внутри ВМ непрерывно меняется память при помощи stress-ng.

![](./images/livemigration-example.ru.png)

В такой ситуации, если сеть ограничивает скорость миграции, можно либо дождаться, когда операция завершится с ошибкой из-за таймаута, либо отменить текущую операцию миграции, удалив объект vmop (`d8 k delete vmop <имя-операции>`), а затем повторно запустить миграцию с использованием флага `--force`, чтобы включить механизм autoconverge.

Пример выполнения миграции той же виртуальной машины с использованием параметра `--force`: здесь хорошо видно, что частота процессора снижается поэтапно, чтобы уменьшить скорость изменения памяти.
...
- Политики сетевой безопасности (NetworkPolicy) не распространяются на дополнительные сетевые интерфейсы.
...
Снимки позволяют зафиксировать текущее состояние ресурса для последующего восстановления или клонирования: снимок диска сохраняет только данные выбранного диска, а снимок виртуальной машины включает в себя параметры ВМ и состояния всех её дисков.
...
{{< alert level="info" >}}
При восстановлении ВМ из снимка связанные с ней диски также восстанавливаются из соответствующих снимков, поэтому в спецификации диска будет указан параметр `dataSource` с ссылкой на нужный снимок диска.
{{< /alert >}}
...
{{< alert level="info" >}}
В процессе клонирования для виртуальной машины и всех её дисков автоматически создаются временные снимки. Именно из этих снимков затем собирается новая ВМ. После завершения процесса клонирования временные снимки автоматически удаляются — их не будет видно в списке ресурсов. Однако внутри спецификации клонируемых дисков будет оставаться ссылка (`dataSource`) на соответствующий снимок, даже если самого снимка уже не существует. Это штатное поведение и не свидетельствует о проблемах: такие ссылки корректны, потому что к моменту запуска клона все необходимые данные уже были перенесены на новые диски.
{{< /alert >}}

## Экспорт данных

Экспортировать диски и снимки дисков виртуальных машин можно с помощью утилиты `d8` (версия 0.20.7 и выше). Для работы этой функции должен быть включен модуль [storage-volume-data-manager](https://deckhouse.ru/modules/storage-volume-data-manager/stable/).

{{< alert level="warning" >}}
Диск не должен использоваться в момент экспорта. Если диск подключен к виртуальной машине, необходимо предварительно остановить ВМ.
{{< /alert >}}
