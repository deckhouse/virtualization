...
- `persistentVolumeClaim` — настройки хранения диска (класс хранилища, размер).
- `dataSource` — источник данных для создания диска (образ, другой диск, снимок).

Если `dataSource` не указан, создаётся пустой диск. В этом случае необходимо указать параметры в блоке `persistentVolumeClaim` (размер и класс хранилища).

Если `dataSource` указан, блок `persistentVolumeClaim` можно опустить. В этом случае:

- Размер диска будет определён автоматически из источника.
- Класс хранилища будет определён автоматически из источника, либо будет использован `StorageClass` по умолчанию на уровне кластера, либо для дисков, если он указан в [настройках модуля](./admin_guide.html#настройки-классов-хранения-для-дисков).
...
- Тип тома: хранилище может поддерживать тома типа файловой системы (FileSystem, например, NFS) или блочные устройства (Block, например, iSCSI, Ceph RBD). Для томов типа файловой системы диск виртуальной машины создаётся в формате qcow2. Некоторые хранилища поддерживают оба типа томов.
- Режим привязки тома (VolumeBindingMode):
  ...
  - `WaitForFirstConsumer` — диск создаётся только после того, как будет подключен к виртуальной машине и будет создан на том узле, на котором будет запущена виртуальная машина.

    ![](images/vd-wffc.ru.png)

При создании диска все параметры (тип тома, формат диска, режим привязки тома и другие настройки) определяются автоматически в зависимости от возможностей выбранного класса хранилища.

{{< alert level="warning" >}}
Создание дисков из iso-образов запрещено!
{{< /alert >}}
