...
{{< alert level="info" >}}
Метки (labels) с исходной ВМ на клон не копируются. Это предотвращает попадание трафика от Service (которые выбирают ВМ по меткам) на клон. Если клон должен входить в сервис, добавьте нужные метки после клонирования (например, с помощью `d8 k label vm <имя-вм> имя-метки=значение`).
{{< /alert >}}
