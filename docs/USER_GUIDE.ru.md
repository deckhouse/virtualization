...
{{< alert level="warning" >}}
Для восстановления виртуальной машины, необходимо, чтобы предыдущая конфигурация ВМ и все её диски были удалены, т.к. восстановлеине предполагает восстановление конфигурации ВМ и всех её дисков на момент создания снимка.
{{< /alert >}}
...
Снимок ВМ можно использовать для создания клона ВМ, либо как шаблон для создания подобных ВМ.

Для этого, необходимо создать ресурс `VirtualMachineRestore` и сконфигурировать в нем блок `.spec.nameReplacements`.
...
Если восстанавливаемый ресурс `VirtualMachineIPAddress` уже присутствует в кластере, он не должен быть использоваться в другой виртуальной машине, а если это ресурс типа Static, его IP-адрес должен совпадать. Восстанавливаемый секрет с автоматизацией также должен полностью соответствовать восстановленному. Несоблюдение этих условий приведет к неудаче восстановления.
