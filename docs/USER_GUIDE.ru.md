...
- Используйте комбинации меток вместо одиночных ограничений. Например, вместо required для одного лейбла (env=prod) используйте несколько preferred условий.
...
- Планируйте резервные узлы для критичных нагрузок. Для ВМ с жесткими требованиями (например, AntiAffinity) предусмотрите запасные узлы, чтобы избежать простоев при сбое.
