...
**Дополнительные сети** — добавляются через блок `.spec.networks`. Каждая сеть может быть подключена несколько раз. 
Допускается подключение/отключение сетей к запущенной ВМ, при этом изменения вступят в силу только после перезагрузки ВМ.
