...
Политика сайзинга в VirtualMachineClass (блок `.spec.sizingPolicies`) задаёт правила для настройки ресурсов виртуальной машины — количества ядер, памяти и доли использования ядер (coreFraction). Эта политика необязательна, но если она есть, ВМ должна строго ей соответствовать, иначе её конфигурацию нельзя будет сохранить.
