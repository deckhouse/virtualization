...
- `Pending` - ВМ только что создана, перезапущена или запущена после остановки и ожидает готовности необходимых ресурсов (дисков, образов, ip-адресов и т.д.).
  - Возможные проблемы:
    - не готовы зависмые ресурсы: диски, образы, классы ВМ, секрет со сценарием начальной конфигурации и пр.
...
- `Starting` - все зависимые ресурсы ВМ - готовы, и система пытается запустить ВМ на одном из узлов кластера.
  - Возможные проблемы:
    ...
    - Превышены квоты.
  - Диагностика:
    - Если запуск затягивается, смотрите `.status.conditions`, условие `type: PodStarted`
- `Running` - ВМ успешно запущена и работает.
  ...
  - Диагностика:
    - Смотрите `.status.conditions`, условие `type: Running`
...
- `Migrating` - ВМ переносится на другой узел кластера (живая миграция).
  - Особенности:
    - Миграция ВМ возможна только для неклокальных дисков, условие `type: Migratable` отображает инфорацию о том может ли ВМ мигрировать или нет.
  - Возможные проблемы:
    - Различия в процессорных инструкциях между узлами (при использовании типов процессоров host или host-passthrough).
    - Несовместимость версий ядер хостовой системы.
    - Отсутствие ресурсов в кластере.
    - Превышение квот.
  - Диагностика:
    - При проблемах смотрите `.status.conditions` условие `type: Migrating`, а также блок `.status.migrationState`
