...
- По умолчанию с каждого узла одновременно передаёт данные только одна виртуальная машина, при этом другие миграции могут готовиться параллельно. Оба лимита настраиваются (см. [Настройка параллелизма живой миграции](#настройка-параллелизма-живой-миграции)).
...
Когда с одного узла одновременно мигрируют несколько виртуальных машин (например, при выводе узла в режим обслуживания), фаза копирования памяти конкурирует за сетевую полосу. Чтобы избежать конкуренции, живая миграция использует два независимых лимита на узле-источнике:

- **Пул подготовки** — `parallelOutboundMigrationsPerNode` (по умолчанию `2`): сколько миграций с узла могут параллельно готовиться (планирование целевого пода, монтирование устройств) до состояния `TargetReady`.
- **Пул передачи данных** — `parallelSyncMigrationsPerNode` (по умолчанию `1`): сколько из подготовленных миграций могут одновременно копировать память. Значение ограничивается сверху значением `parallelOutboundMigrationsPerNode`.

При значениях по умолчанию, пока одна виртуальная машина копирует память, следующая уже подготовлена и начинает передачу сразу после завершения предыдущей, поэтому вывод узла в режим обслуживания идёт конвейером, а не строго последовательно «миграция за миграцией».

Миграция, которая подготовлена, но ожидает свободный слот передачи данных, остаётся в фазе `Pending` с причиной `WaitingForSyncSlot` в своём ресурсе `VirtualMachineOperation`.
