...
- При использовании параметра `memoryPerCore` объем разрешенной памяти рассчитывается как произведение числа ядер на указанный диапазон памяти на одно ядро:
  ...
  Таким образом, при использовании `memoryPerCore` объем разрешенной памяти автоматически масштабируется пропорционально числу ядер, что обеспечивает более гибкое и справедливое распределение ресурсов.
