...
Блок `cores` обязательный и задает диапазон ядер, на которые распространяется правило, описанное в этом же блоке.

Диапазоны [min; max] для параметра cores должны быть строго последовательными и неперекрывающимися.
...
1. Память (`memory`) — указывается либо:
- Минимум и максимум памяти для всех ядер в диапазоне,
...
Далее приведен пример политики с подобными настройками:
