...
2. **Sync** — память виртуальной машины копируется на целевой узел по сети. Именно эта фаза потребляет пропускную способность.
...
Пример — конвейерный вывод узла с сериализованной передачей памяти:
