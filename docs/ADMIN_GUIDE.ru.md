...
- Класс с `type: HostPassthrough`, в этом классе используется физический CPU узла платформы без изменений.
...
- Чтобы создать vCPU конкретного процессора с предварительно определённым набором инструкций, используем тип `type: Model`.
...
Блок `.spec.nodeSelector` опциональный, он позволяет задать узлы на которых будут размещаться ВМ, использующие данный vmclass:
...
{{< alert level="warning" >}}
Изменения блока `.spec.sizingPolicy` также может влиять на виртуальные машины.
Для виртуальных машин, политика сайзинга которых не будет соответствовать новым требованиям политики, условие `SizingPolicyMatched` в блоке `.status.conditions` будет ложным (`status: False`).

При настройке sizingPolicies будьте внимательны и учитывайте [топологию CPU](./USER_GUIDE.ru.md#автоматическая-конфигурация-топологии-cpu) для виртуальных машин.
{{< /alert >}}
...
spec:
  sizingPolicies:
    ...
    # Для диапазона от 17 до 248 ядер возможно использовать от 1 до 2 ГБ оперативной памяти из расчета на одно ядро.
...
2. При необходимосит прервать миграцию, удалите соответствующий ресурс `vmop`, пока он находится в фазе `Pending` или в `InProgress`.
