...
- По умолчанию с каждого узла одновременно передаёт память только одна виртуальная машина. Другие миграции с того же узла могут готовиться параллельно. Подробнее про настройку лимитов описано в разделе [Настройка параллелизма живой миграции](#настройка-параллелизма-живой-миграции).
...
Когда с одного узла одновременно мигрируют несколько виртуальных машин (например, при выводе узла в режим обслуживания), фаза передачи данных (sync) конкурирует за пропускную способность сети. Чтобы избежать конкуренции, живая миграция использует два независимых лимита на узле-источнике:

- Пул подготовки — параметр [.spec.settings.liveMigration.outbound.perNode](/modules/virtualization/alpha/configuration.html#parameters-livemigration-outbound-pernode) (по умолчанию `2`): сколько миграций с узла могут параллельно готовиться (планирование целевого пода, монтирование устройств) до состояния `TargetReady`.
- Пул передачи данных — параметр [.spec.settings.liveMigration.outbound.syncPerNode](/modules/virtualization/alpha/configuration.html#parameters-livemigration-outbound-syncpernode) (по умолчанию `1`): сколько подготовленных миграций могут одновременно находиться в фазе передачи данных (sync). Значение не может превышать `perNode`.

При значениях по умолчанию следующая виртуальная машина уже подготовлена, пока предыдущая передаёт память, и начинает передачу сразу после её завершения. Вывод узла в режим обслуживания идёт конвейером, а не строго «миграция за миграцией».

Миграция, подготовленная, но ожидающая свободный слот передачи данных, остаётся в фазе `Pending` с причиной `WaitingForSyncSlot` в ресурсе
[VirtualMachineOperation](/modules/virtualization/cr.html#virtualmachineoperation).

Настройте лимиты в параметрах модуля:
...
spec:
  settings:
    liveMigration:
      outbound:
        # Максимум миграций, готовящихся параллельно на узле-источнике.
        perNode: 2
        # Максимум миграций, одновременно передающих память на узле-источнике (<= perNode).
        syncPerNode: 1
```
