...
### Мониторинг наличия образов

Платформа может периодически проверять, что образы, хранящиеся в DVCR, по-прежнему существуют. Это помогает обнаружить ситуации, когда образы удаляются из DVCR (вручную или из-за проблем с хранилищем), в то время как соответствующие ресурсы VirtualImage или ClusterVirtualImage всё ещё показывают фазу `Ready`.

При включении система выполняет легковесные запросы к DVCR для проверки существования манифестов образов. Если образ отсутствует, фаза ресурса меняется на `ImageLost`.

Для настройки расписания мониторинга используйте параметр `.spec.settings.dvcr.imageMonitorSchedule`:
...
Параметр принимает cron-выражение. Значение по умолчанию — `0 * * * *` (каждый час). Чтобы отключить периодический мониторинг, укажите пустую строку:
...
Когда образ обнаружен как отсутствующий, статус ресурса изменится:
