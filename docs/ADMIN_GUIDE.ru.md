...
- По умолчанию с каждого узла одновременно готовится и передаёт память только одна виртуальная машина. Подробнее про настройку лимитов описано в разделе [Настройка параллелизма живой миграции](#настройка-параллелизма-живой-миграции).
...
Когда с одного узла одновременно мигрируют несколько виртуальных машин (например, при выводе узла в режим обслуживания), sync-фаза конкурирует за пропускную способность сети. Чтобы избежать конкуренции, живая миграция использует два независимых лимита на узле-источнике, задаваемых через аннотации ModuleConfig `virtualization`:

- Пул подготовки — аннотация `virtualization.deckhouse.io/parallel-outbound-migrations-per-node` (по умолчанию `1`): сколько миграций с узла могут параллельно готовиться (планирование целевого пода, монтирование устройств) до состояния `TargetReady`.
- Пул sync — аннотация `virtualization.deckhouse.io/parallel-sync-migrations-per-node` (по умолчанию `1`): сколько подготовленных миграций могут одновременно находиться в sync-фазе. Миграции сверх лимита ожидают свободный слот.

Увеличение только пула подготовки позволяет выводить узел в режим обслуживания конвейером: следующая виртуальная машина подготавливается, пока предыдущая передаёт память, и начинает передачу сразу после её завершения, при этом одновременно передают память не более `parallel-sync-migrations-per-node` миграций — дополнительная конкуренция за сеть не возникает.

Миграция, подготовленная, но ожидающая свободный sync-слот, остаётся в фазе `Pending` с причиной `WaitingForSyncSlot` в ресурсе [VirtualMachineOperation](/modules/virtualization/cr.html#virtualmachineoperation).

Настройте лимиты через аннотации ModuleConfig:
...
metadata:
  ...
  annotations:
    ...
    # Максимум миграций, одновременно передающих память на узле-источнике.
